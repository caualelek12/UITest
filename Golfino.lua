...
function Peleccos:CreateWindow(o)
    ...
    local function buildPlayerList()
        ...
        local function buildCard(p)
            ...
            -- Jerk Off: plays looping animation + follows target
            mkActionBtn(actFr,"JO",AC,function()
                if _joTarget == p.UserId then
                    ...
                    pcall(function()
                        ...
                        -- Rig-specific animation IDs
                        local isR15 = hum.RigType == Enum.HumanoidRigType.R15
                        -- Official Roblox-owned IDs only (never sanitized)
                        local ids = isR15 and {
                            "rbxassetid://507765000",   -- R15 Jump (looped = dolphin bounce)
                            "rbxassetid://507770548",   -- R15 Cheer
                            "rbxassetid://507771019",   -- R15 Dance
                        } or {
                            "rbxassetid://125750702",   -- R6 Jump (looped = dolphin bounce)
                            "rbxassetid://507770239",   -- Wave fallback
                        }
                        ...
                        for _, id in ipairs(ids) do
                            local ok, track = pcall(function()
                                ...
                                anim.AnimationId = id
                                return animator and animator:LoadAnimation(anim)
                    ...
                    _joConn = RunService.Heartbeat:Connect(function()
                        local myChar  = LP.Character
                        local tgtChar = p.Character
                        if not myChar or not tgtChar then return end
                        local myRoot  = myChar:FindFirstChild("HumanoidRootPart")
                        local tgtRoot = tgtChar:FindFirstChild("HumanoidRootPart")
                        if myRoot and tgtRoot then
                            -- offset in target's local space: behind them (positive Z = back)
                            myRoot.CFrame = tgtRoot.CFrame * CFrame.new(0, 0, 2.5)
                        end
                    end)
                end
            end)
