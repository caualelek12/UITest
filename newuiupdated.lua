--[[
    PeleccosSoftwares v12.5
    Bugs fixed from v12.4:
    1. Notifications showing "?? Notice" with no content:
       WO.Notify = notify stored the function but Window:Notify({...}) passes
       `self` (WO) as the first argument, shifting all real args away.
       Fixed by wrapping: function WO:Notify(o) notify(o) end
    2. Hover highlight stays stuck on Buttons:
       stroke() was called inside MouseEnter/MouseLeave, creating a brand-new
       UIStroke instance every time instead of updating the existing one.
       The old strokes stacked infinitely, leaving highlights stuck.
       Fixed by storing the stroke ref and changing .Color directly.
    3. Hover highlight stays stuck on Sliders:
       Same stroke() re-creation bug on the trackBg inside MouseEnter/MouseLeave.
       Fixed the same way — store trackStroke ref and update its .Color.
    
    Bugs fixed from v12.5 (Ion X patch):
    4. Configs not re-enabling features:
       set(v, true) was called with silent=true on config load, skipping callbacks.
       Fixed by using set(v, false) so callbacks fire and features actually apply.
    5. Mouse buttons not bindable on keybinds/toggles:
       Listening only captured Keyboard UserInputType.
       Fixed by also accepting MouseButton1-5 UserInputTypes.
       Key storage now stores UserInputType for mouse buttons instead of KeyCode.
       Trigger check updated to match both KeyCode and UserInputType.
]]
        ...
        for flag, val in pairs(data) do
            ...
            if info then pcall(function()
                ...
                elseif ftype=="key"    then
                    -- Try KeyCode first, then UserInputType for mouse buttons
                    local success = pcall(function() info.set(Enum.KeyCode[val]) end)
                    if not success then
                        pcall(function() info.set(Enum.UserInputType[val]) end)
                    end
...
function Peleccos:CreateWindow(o)
    ...
    local EGG_BTN = mk("TextButton",{
        ...
        AutoButtonColor=false, BorderSizePixel=0, ZIndex=52, Visible=false, Parent=BAR,
    })
    ...
    local CFGWIN_BTN = mk("TextButton",{
        Text="Configs", TextColor3=C.t1, TextSize=10, Font=Enum.Font.GothamSemibold,
        BackgroundColor3=C.bg4, Size=dim2(0,58,0,BAR_H-6), Position=dim2(1,-128,0,3),
    ...
    local SET_BTN = mk("TextButton",{
        Text="Settings", TextColor3=C.t1, TextSize=10, Font=Enum.Font.GothamSemibold,
        BackgroundColor3=C.bg4, Size=dim2(0,62,0,BAR_H-6), Position=dim2(1,-65,0,3),
        AutoButtonColor=false, BorderSizePixel=0, ZIndex=52, Parent=BAR,
    })
    ...
    -- ── WATERMARK ──────────────────────────────────────────────
    local WM = mk("Frame",{
        Name="Watermark", Size=dim2(0,10,0,20), AutomaticSize=Enum.AutomaticSize.X,
        Position=dim2(0,8,0,BAR_H+5), BackgroundColor3=rgb(8,8,10),
        BackgroundTransparency=0.10, BorderSizePixel=0, ZIndex=30, Parent=SG,
    })
    stroke(WM,C.br1,1)
    ...
    wmLabels.ping  =wmLbl("ping",  "--ms",        false,false); wmSep()
    wmLabels.BuildType =wmLbl("buildtype",CFG.BuildType,true,true)
    draggify(WM,WM)
    ...
    local CONTENT = mk("ScrollingFrame",{
        ...
        ScrollBarImageColor3=AC, CanvasSize=dim2(0,0,0,0), ZIndex=6, Parent=BG_IMG,
    })
    ...
    local swAcc=mk("Frame",{Size=dim2(1,0,0,1),AnchorPoint=Vector2.new(0,1),Position=dim2(0,0,1,0),BackgroundColor3=AC,ZIndex=105,Parent=SW_HDR})
    ...
    local CFGW, CFGW_IMG, cfgwDragHandle = buildWindow(SG,dim2(0,280,0,370),dim2(.5,-290,.5,-185),110,BG_IMAGE,onAC)
    ...
    -- Connect bar buttons AFTER both windows exist
    ...
    function WO:Notify(o) notify(o) end
    ...
    function WO:AddCategory(name)
        ...
        -- FIX 5: keyName now handles both KeyCode and UserInputType
        ...
        function CAT:AddToggle(o5)
            ...
            if hasKB then
                UIS.InputBegan:Connect(function(i,gpe)
                    -- FIX 5: accept keyboard AND mouse buttons when listening
                    if listening and not gpe and (i.UserInputType == Enum.UserInputType.Keyboard or isMouseButton(i.UserInputType)) then
                        -- Store UserInputType for mouse, KeyCode for keyboard
                        kbKey = isMouseButton(i.UserInputType) and i.UserInputType or i.KeyCode
                        kbBtn.Text=keyName(kbKey);kbBtn.TextColor3=AC;kbBtn.BackgroundColor3=C.bg4;listening=false
                    -- FIX 5: trigger check handles both KeyCode and UserInputType
                ...
                UIS.InputEnded:Connect(function(i) if holding and (i.KeyCode==kbKey or i.UserInputType==kbKey) and kbMode=="Hold" then holding=false;set(false) end end)
            end
            -- FIX 4: use silent=false on config load so callback fires and feature actually applies
        ...
        function CAT:AddKeybind(o5)
            ...
            local kb=mk("TextButton",{Text=keyName(key),Size=dim2(0,78,0,17),Position=dim2(1,-82,.5,-8.5),BackgroundColor3=C.bg4,TextColor3=AC,TextSize=9,Font=Enum.Font.GothamBold,AutoButtonColor=false,ZIndex=7,Parent=row}); corner(kb,RC.soft);stroke(kb,C.br0,1)
            ...
            UIS.InputBegan:Connect(function(i,gpe)
                ...
                if listening and not gpe and (i.UserInputType == Enum.UserInputType.Keyboard or isMouseButton(i.UserInputType)) then
                    ...
                    kb.Text=keyName(key);kb.TextColor3=AC;kb.BackgroundColor3=C.bg4;listening=false
                -- FIX 5: trigger check handles both KeyCode and UserInputType
            ...
            if flag then _G[flag]=key;CFGSYS.register(flag,function() return key end,function(k)
                -- FIX 5: config load supports both KeyCode and UserInputType stored as string
