--[[
    PeleccosSoftwares v13.0  (Ion X Edition)
    Visual: GoTTI-style sidebar layout ported to Peleccos backend
    ─────────────────────────────────────────────────────────────
    Backend fixes carried over from v12.5:
    • Config load fires callbacks (silent=false) so features re-enable
    • Mouse buttons bindable on toggles & keybinds (MouseButton1-5)
    • keyName handles both KeyCode and UserInputType
    • Config save/load supports UserInputType keys
]]
        ...
        for flag, val in pairs(data) do
            ...
            if info then pcall(function()
                ...
                elseif ftype=="key"    then
                    local success = pcall(function() info.set(Enum.KeyCode[val]) end)
                    if not success then pcall(function() info.set(Enum.UserInputType[val]) end) end
...
function Peleccos:CreateWindow(o)
    ...
    pcall(function() game:GetService("CoreGui"):FindFirstChild("PeleccosV13"):Destroy() end)
    pcall(function()
        local pg = LP:FindFirstChild("PlayerGui")
        if pg then local x=pg:FindFirstChild("PeleccosV13"); if x then x:Destroy() end end
    end)
    ...
    local SG = mk("ScreenGui", {
        Name="PeleccosV13", ResetOnSpawn=false,
    ...
    local NotifHolder = mk("Frame", {
        Size=dim2(0,256,1,-20), Position=dim2(1,-268,0,10),
        BackgroundTransparency=1, ClipsDescendants=false, Parent=SG,
    })
    layout(NotifHolder, Enum.FillDirection.Vertical, 4, Enum.HorizontalAlignment.Right, Enum.VerticalAlignment.Bottom)
    ...
    local function notify(opts)
        ...
        local Card = mk("Frame", {
            Size=dim2(0,256,0,44), BackgroundColor3=C.BgSideBd,
            BorderSizePixel=0, ClipsDescendants=false, Parent=NotifHolder,
        })
        corner(Card, UDim.new(0,5))
        mk("Frame", {Size=dim2(0,3,1,0), BackgroundColor3=ac, BorderSizePixel=0, Parent=Card})

        local BadgeFr = mk("Frame", {
            Size=dim2(0,18,0,18), Position=dim2(0,10,0,13),
        ...
        mk("TextLabel", {
            Size=dim2(1,-40,0,14), Position=dim2(0,34,0,6),
        ...
        if opts.Desc and opts.Desc ~= "" then
            mk("TextLabel", {
                Size=dim2(1,-40,0,14), Position=dim2(0,34,0,22),
        ...
        task.delay(dur, function()
            tw(Card, {BackgroundTransparency=1, Size=dim2(0,256,0,0)}, 0.2)
            task.wait(0.22); pcall(function() Card:Destroy() end)
        end)
    end

    -- ── Main window (GoTTI layout: 836×616)
    local WIN_W, WIN_H = 836, 560
    local SIDE_W = 190
    ...
    -- ── Sidebar header
    local SideHdr = mk("Frame", {
        Size=dim2(1,0,0,55), BackgroundTransparency=1, Parent=Sidebar,
    })
    -- Logo box
    local LogoBox = mk("Frame", {
        Size=dim2(0,32,0,32), Position=dim2(0,9,0,10),
        BackgroundColor3=rgb(21,21,21), BorderSizePixel=0, Parent=SideHdr,
    })
    corner(LogoBox, UDim.new(0,6))
    local LogoLbl = mk("TextLabel", {
        Size=dim2(1,0,1,0), BackgroundTransparency=1,
        Text="X", TextColor3=AC, TextSize=16, Font=Enum.Font.GothamBold,
        Parent=LogoBox,
    })
    ...
    mk("TextLabel", {
        ...
        TextXAlignment=Enum.TextXAlignment.Left, Parent=SideHdr,
    })
    mk("TextLabel", {
    ...
    local TabScroll = mk("ScrollingFrame", {
        Size=dim2(1,0,1,-160), Position=dim2(0,0,0,62),
    ...
    -- ── Sidebar bottom player card
    mk("Frame", {
        Size=dim2(1,-18,0,1), Position=dim2(0,9,1,-86),
        BackgroundColor3=C.BgGroupH, BorderSizePixel=0, Parent=Sidebar,
    })
    local PCard = mk("Frame", {
        Size=dim2(1,0,0,85), Position=dim2(0,0,1,-85),
    ...
    -- FPS / Ping watermark in player card
    ...
    -- ── Panel scroll area (below subtab bar)
    ...
    -- ── Config manager popup (hidden panel)
    local CfgPanel = mk("Frame", {
        Size=dim2(0,260,0,320), Position=dim2(1,-268,0,50),
        BackgroundColor3=C.BgSideBd, BorderSizePixel=0,
        Visible=false, ZIndex=300, Parent=SG,
    })
    corner(CfgPanel, UDim.new(0,6))
    mk("TextLabel", {
        Size=dim2(1,0,0,32), BackgroundColor3=C.BgGroupH,
        BorderSizePixel=0, Text="Configs", TextColor3=C.Tx0,
        TextSize=12, Font=Enum.Font.GothamBold, ZIndex=301, Parent=CfgPanel,
    })

    local cfgInputFr = mk("Frame", {
        Size=dim2(1,-8,0,24), Position=dim2(0,4,0,36),
        BackgroundColor3=C.BgRow, BorderSizePixel=0, ZIndex=301, Parent=CfgPanel,
    })
    corner(cfgInputFr, UDim.new(0,4))
    local cfgInput = mk("TextBox", {
        ...
        ZIndex=302, Parent=cfgInputFr,
    })

    local cfgListSF = mk("ScrollingFrame", {
        Size=dim2(1,-8,1,-100), Position=dim2(0,4,0,66),
        BackgroundTransparency=1, ScrollBarThickness=2,
        ScrollBarImageColor3=C.Toggle, BorderSizePixel=0,
        CanvasSize=dim2(0,0,0,0), ZIndex=301, Parent=CfgPanel,
    })
    local cfgLL = layout(cfgListSF, Enum.FillDirection.Vertical, 3)
    cfgLL:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        task.defer(function() if cfgListSF and cfgListSF.Parent then
            cfgListSF.CanvasSize=dim2(0,0,0,cfgLL.AbsoluteContentSize.Y+6) end end)
    end)
    ...
    local function refreshCfgList()
        ...
        if #list == 0 then
            mk("TextLabel", {
                ...
                Text="No configs yet.", TextColor3=C.Tx2,
                TextSize=10, Font=Enum.Font.Gotham, ZIndex=302, Parent=cfgListSF,
            })
            return
        end
        for _, name in ipairs(list) do
            local row = mk("Frame", {
                Size=dim2(1,0,0,28), BackgroundColor3=C.BgRow,
                BorderSizePixel=0, ZIndex=302, Parent=cfgListSF,
            })
            corner(row, UDim.new(0,4))
            mk("TextLabel", {
                Size=dim2(1,-86,1,0), Position=dim2(0,6,0,0),
                BackgroundTransparency=1, Text=name, TextColor3=C.Tx1,
                TextSize=11, Font=Enum.Font.Gotham,
                TextXAlignment=Enum.TextXAlignment.Left,
                TextTruncate=Enum.TextTruncate.AtEnd, ZIndex=303, Parent=row,
            })
            local function mkSmBtn(txt, xo, col, cb2)
                local b = mk("TextButton", {
                    Size=dim2(0,24,0,18), Position=dim2(1,xo,0.5,-9),
                    BackgroundColor3=col, Text=txt, TextColor3=C.Tx0,
                    TextSize=9, Font=Enum.Font.GothamBold,
                    AutoButtonColor=false, BorderSizePixel=0, ZIndex=303, Parent=row,
                })
                corner(b, UDim.new(0,3))
                b.MouseButton1Click:Connect(cb2)
            end
            mkSmBtn("L", -82, rgb(30,80,50), function()
                if CFGSYS.load(name) then
                    CFG.ConfigName=name
                    notify({Title="Loaded",Desc=name,Type="Success",Duration=2})
                else
                    notify({Title="Load Failed",Desc=name,Type="Error",Duration=2})
                end
            end)
            mkSmBtn("S", -54, rgb(30,50,80), function()
                if CFGSYS.save(name) then notify({Title="Saved",Desc=name,Type="Success",Duration=2})
                else notify({Title="Save Failed",Type="Error",Duration=2}) end
            end)
            mkSmBtn("X", -26, rgb(80,30,30), function()
                CFGSYS.delete(name); refreshCfgList()
                notify({Title="Deleted",Desc=name,Type="Warning",Duration=2})
            end)
        end
    end

    -- Create / folder buttons
    local cfgBtnRow = mk("Frame", {
        Size=dim2(1,-8,0,24), Position=dim2(0,4,1,-28),
        BackgroundTransparency=1, ZIndex=301, Parent=CfgPanel,
    })
    layout(cfgBtnRow, Enum.FillDirection.Horizontal, 4)
    local function mkCfgBtn(txt, cb2)
        local b = mk("TextButton", {
            Size=dim2(0.5,-2,1,0), BackgroundColor3=C.BgRow,
            Text=txt, TextColor3=C.Tx1, TextSize=10, Font=Enum.Font.GothamBold,
            AutoButtonColor=false, BorderSizePixel=0, ZIndex=302, Parent=cfgBtnRow,
        })
        corner(b, UDim.new(0,4))
        b.MouseButton1Click:Connect(cb2)
        return b
    end
    mkCfgBtn("+ Create", function()
        local name = cfgInput.Text ~= "" and cfgInput.Text or ("Config"..tostring(#CFGSYS.list()+1))
        name = name:gsub("[^%w%-%_]","_"); cfgInput.Text=""
        if CFGSYS.save(name) then
            notify({Title="Created",Desc=name,Type="Success",Duration=2}); refreshCfgList()
        else
            notify({Title="Create Failed",Type="Error",Duration=3})
        end
    end)
    mkCfgBtn("Open Folder", function() CFGSYS.openDir() end)
    ...
    local function regUnder(u, tk, sk) table.insert(DynUnders, {under=u, tabKey=tk, subKey=sk}) end
    ...
    local function applyAccent(c)
        ...
        for _, e in ipairs(DynUnders) do
            pcall(function()
                if e.under.Visible then e.under.BackgroundColor3=c end
            end)
        end
        for k, btn in pairs(Tabs) do
            if k==ActiveTab then
                local ind=btn:FindFirstChild("Ind")
                if ind then pcall(function() ind.BackgroundColor3=c end) end
                local ico=btn:FindFirstChild("Ico")
                if ico then pcall(function() ico.TextColor3=c end) end
            end
        end
        for tabKey, stMap in pairs(SubTabsMap) do
            for sk, st in pairs(stMap) do
                if ActiveTab==tabKey and ActiveSubs[tabKey]==sk then
                    pcall(function() st.lbl.TextColor3=c end)
                end
            end
        end
    end
    ...
    local function SetActiveTab(key)
        -- hide all
        for k2, scr in pairs(SubTabScrolls) do scr.Visible=false end
    ...
    -- ── Window object
    local WO = {_categories={}, _activeCat=nil, _cfgsys=CFGSYS}

    function WO:Notify(opts) notify(opts) end

    function WO:AddCategory(name)
        -- Each category = a sidebar tab button + subtab scroll
        ...
        mk("TextLabel", {
            Name="Lbl", Size=dim2(1,-36,1,0), Position=dim2(0,30,0,0),
        ...
        Btn.MouseEnter:Connect(function()
            if catKey~=ActiveTab then
                ...
                local lbl=Btn:FindFirstChild("Lbl"); if lbl then lbl.TextColor3=C.Tx1 end
            end
        end)
        Btn.MouseLeave:Connect(function()
            if catKey~=ActiveTab then
                tw(Btn, {BackgroundTransparency=1}, 0.1)
                local lbl=Btn:FindFirstChild("Lbl"); if lbl then lbl.TextColor3=C.Tx2 end
            end
        end)

        Tabs[catKey] = Btn

        -- Subtab scroll inside subtab bar
        ...
        local StLayout = layout(StScroll, Enum.FillDirection.Horizontal, 0,
            Enum.HorizontalAlignment.Left, Enum.VerticalAlignment.Center)
        padding(StScroll, 0, 0, 0, 12)
        ...
        -- CAT object (acts like a tab, can add subtabs via AddSection or direct widgets)
        ...
        -- ── AddSubTab: creates a subtab strip button + panel frame
        -- Returns a panel parent (Frame) to add widgets into
        local function addSubTab(label)
            ...
            regUnder(StUnder, catKey, subKey)
            ...
            StBtn.MouseButton1Click:Connect(function()
                for sk2, st2 in pairs(SubTabsMap[catKey]) do
                    ...
                    st2.under.Visible=false; st2.btn.BackgroundTransparency=1
                end
        ...
        -- ── Default panel (no subtabs scenario — one auto subtab per category)
        local _defaultPanel = nil
        local function getDefaultPanel()
            if not _defaultPanel then
                _defaultPanel = addSubTab(name)
            end
            return _defaultPanel
        end

        -- ── Section builder (creates a grouped card inside a panel)
        local function mkSection(parent, title, order)
            ...
            local Hdr = mk("Frame", {
                Size=dim2(1,0,0,28), BackgroundTransparency=1, Parent=Inner,
            })
            mk("TextLabel", {
        ...
        local function mkRow(parent, h)
            local r = mk("Frame", {
                Size=dim2(1,0,0,h or 28), BackgroundTransparency=1, Parent=parent,
            })
            return r
        end
        ...
        function CAT:AddToggle(o5)
            ...
            local row = mkRow(panel, hasKB and 32 or 28)

            -- Label
            local Lbl = mk("TextLabel", {
                Size=dim2(1,-(hasKB and 154 or 36),1,0), Position=dim2(0,0,0,0),
            ...
            if hasKB then
                modeBtn = mk("TextButton", {
                    Size=dim2(0,16,0,14), Position=dim2(1,-152,0.5,-7),
                    BackgroundColor3=C.BgRow, TextColor3=C.Tx2,
                    Text="T", TextSize=8, Font=Enum.Font.GothamBold,
                    AutoButtonColor=false, BorderSizePixel=0, Parent=row,
                })
                corner(modeBtn, UDim.new(0,3))
                ...
                modeBtn.MouseButton2Click:Connect(function()
                    ...
                    notify({Title="Bind Mode",Desc=nm..": "..kbMode,Type="Info",Duration=1.5})
                end)

                kbBtn = mk("TextButton", {
                    Size=dim2(0,62,0,14), Position=dim2(1,-126,0.5,-7),
                    BackgroundColor3=C.BgRow, TextColor3=AC,
                    Text=keyName(kbKey), TextSize=9, Font=Enum.Font.GothamBold,
                    AutoButtonColor=false, BorderSizePixel=0, Parent=row,
                })
                corner(kbBtn, UDim.new(0,3))
                ...
                kbBtn.MouseButton1Click:Connect(function()
                    ...
                    kbBtn.BackgroundColor3=rgb(28,24,10)
                end)
            end
            ...
            if hasKB then
                UIS.InputBegan:Connect(function(i, gpe)
                    if listening and not gpe and (i.UserInputType==Enum.UserInputType.Keyboard or isMouseButton(i.UserInputType)) then
                        kbKey = isMouseButton(i.UserInputType) and i.UserInputType or i.KeyCode
                        kbBtn.Text=keyName(kbKey); kbBtn.TextColor3=AC
                        kbBtn.BackgroundColor3=C.BgRow; listening=false
                    elseif not listening and not gpe and (i.KeyCode==kbKey or i.UserInputType==kbKey) and kbKey~=Enum.KeyCode.Unknown then
                ...
                UIS.InputEnded:Connect(function(i)
                    if holding and (i.KeyCode==kbKey or i.UserInputType==kbKey) and kbMode=="Hold" then
                        holding=false; set(false)
                    end
                end)
            end

            -- FIX: silent=false so config load fires callbacks
            if flag then _G[flag]=val; CFGSYS.register(flag,function() return val end,function(v) set(v,false) end,"bool") end
        ...
        function CAT:AddSlider(o5)
            ...
            local cb=o5.Callback or function() end; local val=math.clamp(o5.Default or mn,mn,mx)

            local panel = getDefaultPanel()
            local wrap = mk("Frame", {
                Size=dim2(1,0,0,44), BackgroundTransparency=1, Parent=panel,
            })
            ...
            mk("TextLabel", {
                Size=dim2(0.55,0,1,0), BackgroundTransparency=1, Text=nm,
                TextColor3=C.Tx1, TextSize=13, Font=Enum.Font.GothamBold,
                TextXAlignment=Enum.TextXAlignment.Left, Parent=topRow,
            })
            local ValBox = mk("Frame", {
                Size=dim2(0,36,0,14), Position=dim2(1,-38,0.5,-7),
            ...
            local Track = mk("Frame", {
                Size=dim2(1,-46,0,5), Position=dim2(0,0,0.5,-2.5),
            ...
            local minusBtn = mk("TextButton", {
                Size=dim2(0,18,0,18), Position=dim2(1,-42,0.5,-9),
            ...
            local plusBtn = mk("TextButton", {
                Size=dim2(0,18,0,18), Position=dim2(1,-20,0.5,-9),
                BackgroundColor3=C.BgRow, Text="+", TextColor3=C.Tx1,
                TextSize=14, Font=Enum.Font.GothamBold,
                AutoButtonColor=false, BorderSizePixel=0, Parent=trackRow,
            })
            ...
            Hit.InputBegan:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 then
                    dragging=true; sv(posToVal(i.Position.X))
                end
            end)
            UIS.InputChanged:Connect(function(i)
            ...
            minusBtn.MouseButton1Click:Connect(function()
                local mult=UIS:IsKeyDown(Enum.KeyCode.LeftShift) and 10 or 1; sv(val-step*mult)
            end)
        ...
        function CAT:AddDropdown(o5)
            ...
            local panel = getDefaultPanel()
            local wrap = mk("Frame", {
                Size=dim2(1,0,0,46), BackgroundTransparency=1, Parent=panel,
            })
            mk("TextLabel", {
            ...
            local function buildDD(ov)
                ...
                local sc=mk("ScrollingFrame",{Size=dim2(1,0,1,0),BackgroundTransparency=1,
                    ScrollBarThickness=2,ScrollBarImageColor3=C.Toggle,CanvasSize=dim2(0,0,0,0),
                    ZIndex=221,Parent=pan})
                layout(sc,Enum.FillDirection.Vertical,2); padding(sc,4,4,4,4)
                local scLL=sc:FindFirstChildOfClass("UIListLayout")
                if scLL then scLL:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                    task.defer(function() if sc and sc.Parent then sc.CanvasSize=dim2(0,0,0,scLL.AbsoluteContentSize.Y+8) end end)
                end) end
                for _,op in ipairs(opts) do
                    ...
                    local ob=mk("TextButton",{Size=dim2(1,0,0,24),Text=op,
                        ...
                        TextSize=12,Font=Enum.Font.Gotham,AutoButtonColor=false,ZIndex=222,Parent=sc})
                    corner(ob,UDim.new(0,4))
                    ob.MouseEnter:Connect(function()
                        if not(multi and table.find(msel,op)) and op~=sel then
                            tw(ob,{BackgroundColor3=C.BgGroupH,TextColor3=C.Tx0},0.08)
                        end
                    end)
                    ob.MouseLeave:Connect(function()
                        local s2=multi and table.find(msel,op)~=nil or op==sel
                        ob.BackgroundColor3=s2 and AC or C.BgRow
                        ob.TextColor3=s2 and C.White or C.Tx1
                    end)
                    ob.MouseButton1Click:Connect(function()
                        if multi then
                            ...
                            sel=op; SelLbl.Text=op; pcall(cb,op)
            ...
            if flag then _G[flag]=sel; CFGSYS.register(flag,function() return sel end,function(v) sel=v;SelLbl.Text=v;if flag then _G[flag]=v end end,"string") end
            local r={Value=sel}
        ...
        function CAT:AddKeybind(o5)
            ...
            Kb.MouseButton1Click:Connect(function()
                if listening then return end
                listening=true; Kb.Text="..."; Kb.TextColor3=rgb(255,185,0)
                Kb.BackgroundColor3=rgb(28,24,10)
            end)
            UIS.InputBegan:Connect(function(i, gpe)
                if listening and not gpe and (i.UserInputType==Enum.UserInputType.Keyboard or isMouseButton(i.UserInputType)) then
                    key=isMouseButton(i.UserInputType) and i.UserInputType or i.KeyCode
                    Kb.Text=keyName(key); Kb.TextColor3=AC; Kb.BackgroundColor3=C.BgRow; listening=false
                elseif not listening and not gpe and (i.KeyCode==key or i.UserInputType==key) and key~=Enum.KeyCode.Unknown then
        ...
        function CAT:AddButton(o5)
            o5=o5 or {}
            local nm=o5.Name or "Button"; local cb=o5.Callback or function() end
            local panel = getDefaultPanel()
            local Btn = mk("TextButton", {
                Size=dim2(1,0,0,28), BackgroundColor3=C.BgGroupH,
                Text=nm, TextColor3=C.Tx1, TextSize=13, Font=Enum.Font.GothamBold,
            ...
            Btn.MouseButton1Click:Connect(function() task.delay(0.18,function() tw(Btn,{BackgroundColor3=C.BgGroupH,TextColor3=C.Tx1},0.14) end); pcall(cb) end)
            onAC(function(c) end) -- placeholder for accent-colored buttons if needed
        ...
        function CAT:AddSeparator()
            ...
            local sep = mk("Frame", {
                ...
                BackgroundTransparency=0.4, Parent=panel,
            })
            mk("UIGradient", {
        ...
        function CAT:AddTextbox(o5)
            o5=o5 or {}
            local nm=o5.Name or "Input"; local cb=o5.Callback or function() end; local flag=o5.Flag
            local panel = getDefaultPanel()
            local wrap = mk("Frame", {Size=dim2(1,0,0,46), BackgroundTransparency=1, Parent=panel})
            mk("TextLabel", {
                Size=dim2(1,0,0,14), BackgroundTransparency=1, Text=nm,
                TextColor3=C.Tx2, TextSize=10, Font=Enum.Font.Gotham,
                TextXAlignment=Enum.TextXAlignment.Left, Parent=wrap,
            ...
            tb.FocusLost:Connect(function() tw(ifrm,{BackgroundColor3=C.BgRow},0.1); pcall(cb,tb.Text); if flag then _G[flag]=tb.Text end end)
            ...
            Sw.MouseButton1Click:Connect(function()
                ...
                if open then
                    openOV(function(ov)
                        ...
                        -- SV box
                        local svbg=mk("Frame",{Size=dim2(1,-12,0,80),Position=dim2(0,6,0,6),
                        ...
                        local hueBar=mk("TextButton",{AutoButtonColor=false,Text="",
                            Size=dim2(1,-12,0,8),Position=dim2(0,6,0,92),ZIndex=221,Parent=pan})
                        ...
                        local prev=mk("Frame",{Size=dim2(1,-12,0,12),Position=dim2(0,6,0,106),
        ...
        function CAT:AddProgressBar(o5)
            o5=o5 or {}
            local nm=o5.Name or "Progress"; local maxV=o5.Max or 100; local cur=math.clamp(o5.Default or 0,0,maxV)
            local panel = getDefaultPanel()
            local wrap = mk("Frame", {Size=dim2(1,0,0,38), BackgroundTransparency=1, Parent=panel})
            local topRow = mk("Frame", {Size=dim2(1,0,0,16), BackgroundTransparency=1, Parent=wrap})
            mk("TextLabel",{Size=dim2(1,-40,1,0),BackgroundTransparency=1,Text=nm,TextColor3=C.Tx1,TextSize=12,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,Parent=topRow})
            local pctLbl=mk("TextLabel",{Size=dim2(0,36,1,0),Position=dim2(1,-38,0,0),BackgroundTransparency=1,Text=math.round(cur/maxV*100).."%",TextColor3=AC,TextSize=11,Font=Enum.Font.Code,Parent=topRow})
            regText(pctLbl)
            local trk=mk("Frame",{Size=dim2(1,0,0,4),Position=dim2(0,0,0,22),BackgroundColor3=C.BgRow,BorderSizePixel=0,Parent=wrap}); corner(trk,UDim.new(1,0))
            local fill2=mk("Frame",{Size=dim2(cur/maxV,0,1,0),BackgroundColor3=AC,BorderSizePixel=0,Parent=trk}); corner(fill2,UDim.new(1,0)); regFill(fill2)
            local r={Value=cur}
            function r:Set(v) v=math.clamp(v,0,maxV);cur=v;tw(fill2,{Size=dim2(v/maxV,0,1,0)},0.2);pctLbl.Text=math.round(v/maxV*100).."%" end
            function r:Get() return cur end; return r
        end

        -- Expose addSubTab so power users can create sub-pages
        CAT.AddSubTab = function(self2, label)
            return addSubTab(label)
        end

        -- Initial activation
        if isFirst then
            task.defer(function() SetActiveTab(catKey) end)
        end

        return CAT
    end

    -- ── Settings / Config button in content header
    -- (Attach a small config button to the subtab bar area)
    local cfgToggleBtn = mk("TextButton", {
        Size=dim2(0,64,0,28), Position=dim2(1,-72,0,8),
        BackgroundColor3=C.BgGroupH, Text="Configs", TextColor3=C.Tx1,
        TextSize=10, Font=Enum.Font.GothamBold,
        AutoButtonColor=false, BorderSizePixel=0, ZIndex=55, Parent=Main,
    })
    corner(cfgToggleBtn, UDim.new(0,4))
    cfgToggleBtn.MouseButton1Click:Connect(function()
        CfgPanel.Visible=not CfgPanel.Visible
        if CfgPanel.Visible then refreshCfgList() end
    end)

    -- Accent color button
    local accentBtn = mk("TextButton", {
        Size=dim2(0,18,0,18), Position=dim2(1,-98,0,13),
        BackgroundColor3=AC, Text="", AutoButtonColor=false,
        BorderSizePixel=0, ZIndex=55, Parent=Main,
    })
    corner(accentBtn, UDim.new(1,0))
    onAC(function(c) accentBtn.BackgroundColor3=c end)
    accentBtn.MouseButton1Click:Connect(function()
        openOV(function(ov)
            local ch2,cs2,cv2=Color3.toHSV(AC)
            local pw,ph=200,140
            local ap=accentBtn.AbsolutePosition
            local px=math.min(ap.X, SG.AbsoluteSize.X-pw-10)
            local py=ap.Y+accentBtn.AbsoluteSize.Y+6
            if py+ph>SG.AbsoluteSize.Y-10 then py=ap.Y-ph-6 end
            local pan=mk("Frame",{Size=dim2(0,pw,0,ph),Position=dim2(0,px,0,py),
                BackgroundColor3=C.BgSideBd,ZIndex=220,Parent=ov}); corner(pan,UDim.new(0,6))
            local svbg=mk("Frame",{Size=dim2(1,-12,0,80),Position=dim2(0,6,0,6),
                BackgroundColor3=Color3.fromHSV(ch2,1,1),ZIndex=221,Parent=pan}); corner(svbg,UDim.new(0,4))
            local wg=mk("Frame",{Size=dim2(1,0,1,0),BackgroundColor3=rgb(255,255,255),ZIndex=222,Parent=svbg}); corner(wg,UDim.new(0,4))
            mk("UIGradient",{Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0),NumberSequenceKeypoint.new(1,1)}),Parent=wg})
            local bgf=mk("Frame",{Size=dim2(1,0,1,0),BackgroundColor3=rgb(0,0,0),ZIndex=223,Parent=svbg}); corner(bgf,UDim.new(0,4))
            mk("UIGradient",{Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,1),NumberSequenceKeypoint.new(1,0)}),Rotation=90,Parent=bgf})
            local svc=mk("TextButton",{AutoButtonColor=false,Text="",AnchorPoint=Vector2.new(.5,.5),
                Size=dim2(0,9,0,9),Position=dim2(cs2,0,1-cv2,0),BackgroundColor3=rgb(255,255,255),ZIndex=226,Parent=svbg}); corner(svc,UDim.new(1,0))
            local hueBar=mk("TextButton",{AutoButtonColor=false,Text="",Size=dim2(1,-12,0,8),
                Position=dim2(0,6,0,92),ZIndex=221,Parent=pan}); corner(hueBar,UDim.new(1,0))
            ...
            local hueCur=mk("Frame",{AnchorPoint=Vector2.new(.5,.5),Size=dim2(0,8,1,2),Position=dim2(ch2,0,.5,0),BackgroundColor3=rgb(255,255,255),ZIndex=223,Parent=hueBar}); corner(hueCur,UDim.new(0,2))
            local prev=mk("Frame",{Size=dim2(1,-12,0,12),Position=dim2(0,6,0,106),BackgroundColor3=Color3.fromHSV(ch2,cs2,cv2),ZIndex=221,Parent=pan}); corner(prev,UDim.new(0,4))
    ...
    function WO:Toggle() _vis=not _vis; Main.Visible=_vis; CfgPanel.Visible=false end
    function WO:Destroy() SG:Destroy() end
    function WO:SetConfigName(n) CFG.ConfigName=n end
