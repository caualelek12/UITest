...
function Peleccos:CreateWindow(o)
    ...
    function WO:AddCategory(name)
        ...
        function CAT:AddButton(o5)
            ...
            corner(face, RC.soft); stroke(face, C.br1, 1)
            local lbl = mk("TextLabel",{
                Text=nm, Size=dim2(1,0,1,0), BackgroundTransparency=1,
                TextColor3=C.t1, TextSize=12, Font=Enum.Font.GothamSemibold, ZIndex=8, Parent=face,
            })
            -- left accent pip (invisible until hover)
            local pip = mk("Frame",{
                Size=dim2(0,2,0,14), Position=dim2(0,0,.5,-7),
                BackgroundColor3=AC, BackgroundTransparency=1, ZIndex=8, Parent=face,
            })
            corner(pip, RC.pill)
        ...
        function CAT:AddSlider(o5)
            ...
            local fill = mk("Frame",{Size=dim2(pct,0,1,0),BackgroundColor3=AC,ZIndex=8,Parent=trackBg})
            corner(fill, RC.pill); onAC(function(c) fill.BackgroundColor3 = c end)
            local kn = mk("Frame",{
                AnchorPoint=Vector2.new(.5,.5), Size=dim2(0,13,0,13),
                Position=dim2(pct,0,.5,0), BackgroundColor3=C.t0,
            ...
            UIS.InputEnded:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 then
                    dragging=false; tw(vLbl,{TextColor3=C.t2},Enum.EasingStyle.Quint,0.25)
                end
            end)
