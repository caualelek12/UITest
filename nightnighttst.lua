...
function Peleccos:CreateWindow(o)
    ...
    function WO:AddCategory(name, _pinned)
        ...
        function CAT:_AddKeybind(o5, parentOverride)
            ...
            Kb.MouseEnter:Connect(function() if not listen then tw(Kb,{BackgroundColor3=C.BgGroupH},0.08) end end)
            Kb.MouseLeave:Connect(function() if not listen then tw(Kb,{BackgroundColor3=C.BgRow},0.1) end end)
            ...
            local function startListening()
                ...
                _kbWidgetConn = UIS.InputBegan:Connect(function(i,gpe)
                    if not listen then
                        -- fire callback on keybind press
                        if not gpe and key~=Enum.KeyCode.Unknown and (i.KeyCode==key or i.UserInputType==key) then
                            pcall(cb)
                        end
                        return
                    end
