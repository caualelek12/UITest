...
function Peleccos:CreateWindow(o)
    ...
    function WO:AddCategory(name, _pinned)
        ...
        function CAT:_AddToggle(o5, parentOverride)
            ...
            -- Checkbox: plain filled square, no text, no inner frame
            ...
            -- Only show right-click popup if caller explicitly passed a Keybind option
        ...
        function CAT:_AddDropdown(o5, parentOverride)
            ...
            onFont(function(mf,_) NmLbl.Font=mf end)
            local Hd = mk("TextButton",{Size=dim2(1,0,0,24),Position=dim2(0,0,0,15),
                BackgroundColor3=C.BgRow,Text="",AutoButtonColor=false,
                BorderSizePixel=0,Parent=wrap})
            Hd.MouseEnter:Connect(function() tw(Hd,{BackgroundColor3=C.BgGroupH},0.08) end)
            Hd.MouseLeave:Connect(function() tw(Hd,{BackgroundColor3=C.BgRow},0.1) end)
            local SelLbl = mk("TextLabel",{Size=dim2(1,-22,1,0),Position=dim2(0,7,0,0),
            ...
            local function buildDD(ov)
                ...
                local ITEM_H = 20
                local lh     = math.min(#opts2 * ITEM_H + 4, 160)
                -- match the header width exactly
                local pw  = as.X
                local px  = math.min(ap.X, SG.AbsoluteSize.X - pw - 10)
                local py  = ap.Y + as.Y + 3
                if py + lh > SG.AbsoluteSize.Y - 10 then py = ap.Y - lh - 3 end

                -- Start collapsed (height 0) then slide open
                local pan = mk("Frame",{
                    Size=dim2(0,pw,0,0), Position=dim2(0,px,0,py),
                    BackgroundColor3=C.BgMain, BorderSizePixel=1,
                    ClipsDescendants=true, ZIndex=220, Parent=ov,
                })
                tw(pan, {Size=dim2(0,pw,0,lh)}, 0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
                ...
                local function refreshItems()
                    for _, info in ipairs(itemFrames) do
                        ...
                        info.bar.Visible          = isSel2
                        info.lbl.TextColor3       = isSel2 and C.Tx0 or C.Tx1
                        info.frame.BackgroundColor3 = isSel2 and C.BgGroupH or C.BgRow
                    end
                end
                ...
                for _, op in ipairs(opts2) do
                    ...
                    local item = mk("Frame",{
                        ...
                        BackgroundColor3=isSel and C.BgGroupH or C.BgRow,
                        BorderSizePixel=0, ZIndex=222, Parent=sc,
                    })
                    -- left accent bar for selected
                    local bar = mk("Frame",{
                        Size=dim2(0,2,1,0), BackgroundColor3=AC,
                        BorderSizePixel=0, Visible=isSel, ZIndex=223, Parent=item,
                    })
                    onAC(function(c) bar.BackgroundColor3=c end)
                    local lbl = mk("TextLabel",{
                        Size=dim2(1,-12,1,0), Position=dim2(0,8,0,0),
                    ...
                    hitBtn.MouseEnter:Connect(function()
                        item.BackgroundColor3 = C.BgGroupH
                    end)
                    hitBtn.MouseLeave:Connect(function()
                        local s = multi and table.find(msel,op)~=nil or op==sel
                        item.BackgroundColor3 = s and C.BgGroupH or C.BgRow
                    end)
                    ...
                    table.insert(itemFrames, {frame=item, bar=bar, lbl=lbl, op=op})
                end
            end
