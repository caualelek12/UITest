...
function Peleccos:CreateWindow(o)
    ...
    local wExec = mk("TextLabel", {
        Size=dim2(0.65,0,0,12), Position=dim2(0,8,0,68),
        BackgroundTransparency=1, Text=_executor,
        TextColor3=C.TxSub, TextSize=9, Font=Enum.Font.Code,
        TextXAlignment=Enum.TextXAlignment.Left, Parent=PCard,
    })
    local _keyDisplay = tostring(KEY):gsub("Enum%.KeyCode%.",""):gsub("Enum%.UserInputType%.","")
    local wKey = mk("TextLabel", {
        Size=dim2(0.35,0,0,12), Position=dim2(0.65,0,0,68),
        BackgroundTransparency=1, Text="[".. _keyDisplay .."]",
        TextColor3=C.TxSub, TextSize=9, Font=Enum.Font.Code,
        TextXAlignment=Enum.TextXAlignment.Left, Parent=PCard,
    })
    RunService.Heartbeat:Connect(function()
    ...
    UIS.InputBegan:Connect(function(i, gpe)
        if not gpe and i.KeyCode == KEY then
            if _vis then
                tw(Main, {Size=dim2(0,WIN_W*0.92,0,WIN_H*0.92)}, 0.12, Enum.EasingStyle.Quint)
                ...
                Main.Size = dim2(0,WIN_W*0.92,0,WIN_H*0.92)
    ...
    function WO:Toggle()
        if _vis then
            tw(Main,{Size=dim2(0,WIN_W*0.92,0,WIN_H*0.92)},0.12,Enum.EasingStyle.Quint)
            task.delay(0.13,function() _vis=false; Main.Visible=false; Main.Size=dim2(0,WIN_W,0,WIN_H) end)
        else
            _vis=true; Main.Visible=true
            Main.Size=dim2(0,WIN_W*0.92,0,WIN_H*0.92)
            tw(Main,{Size=dim2(0,WIN_W,0,WIN_H)},0.2,Enum.EasingStyle.Quint,Enum.EasingDirection.Out)
    ...
    function WO:AddCategory(name, _pinned)
        ...
        local Lbl = mk("TextLabel",{Name="Lbl",Size=dim2(1,-16,1,0),Position=dim2(0,12,0,0),
            ...
            TextSize=12,Font=MenuFont,TextXAlignment=Enum.TextXAlignment.Left,Parent=Btn})
        ...
        function CAT:_AddToggle(o5, parentOverride)
            ...
            local Lbl = mk("TextLabel",{Size=dim2(1,-20,1,0),BackgroundTransparency=1,Text=nm,
                TextColor3=val and C.Tx0 or C.Tx2,TextSize=13,Font=MenuFont,
        ...
        function CAT:_AddSlider(o5, parentOverride)
            ...
            regFill(Fill)
            -- End cap marker (right wall)
            mk("Frame",{Size=dim2(0,2,0,11),Position=dim2(1,0,0.5,-5),
                BackgroundColor3=C.Tx2,BorderSizePixel=0,Parent=Track})
        ...
        function CAT:_AddKeybind(o5, parentOverride)
            ...
            local NmLbl = mk("TextLabel",{Size=dim2(1,-88,1,0),BackgroundTransparency=1,Text=nm,
                TextColor3=C.Tx2,TextSize=13,Font=MenuFont,
                TextXAlignment=Enum.TextXAlignment.Left,Parent=row})
            onFont(function(mf,_) NmLbl.Font=mf end)
    ...
    do
        ...
        function WO:Toggle()
            if _vis then
                tw(Main,{Size=dim2(0,WIN_W*0.92,0,WIN_H*0.92)},0.12,Enum.EasingStyle.Quint)
                task.delay(0.13,function() _vis=false; Main.Visible=false; Main.Size=dim2(0,WIN_W,0,WIN_H); hudUpdateVisibility() end)
            else
                _vis=true; Main.Visible=true
                Main.Size=dim2(0,WIN_W*0.92,0,WIN_H*0.92)
                tw(Main,{Size=dim2(0,WIN_W,0,WIN_H)},0.2,Enum.EasingStyle.Quint,Enum.EasingDirection.Out)
