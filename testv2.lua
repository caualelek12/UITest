...
-- FIX 1: notify takes a plain table — no self involved.
-- The old WO.Notify = notify broke because Window:Notify({...})
-- passes (WO, {table}) not ({table}). Fixed below in WO definition.
    ...
    local function saveCfg(name)
        ...
        for flag, info in pairs(_flags) do
            pcall(function()
                ...
                elseif ftype=="key"    then data[flag]=tostring(v):gsub("Enum%.KeyCode%.","")
    ...
    local function loadCfg(name)
        ...
        for flag, val in pairs(data) do
            ...
            if info then pcall(function()
                ...
                elseif ftype=="key"    then pcall(function() info.set(Enum.KeyCode[val]) end)
...
function Peleccos:CreateWindow(o)
    ...
    -- FIX 1: proper method wrapper so self isn't passed to notify()
    ...
    function WO:AddCategory(name)
        ...
        function CAT:AddButton(o5)
            ...
            -- FIX 2: store the single stroke ref — never call stroke() again on this face
            ...
            hbtn.MouseEnter:Connect(function()
                ...
                faceStroke.Color=AC  -- update existing stroke, don't create new one
            end)
            hbtn.MouseLeave:Connect(function()
                ...
                faceStroke.Color=C.br1  -- restore existing stroke
            end)
        ...
        function CAT:AddSlider(o5)
            ...
            -- FIX 3: store single stroke ref for trackBg, update .Color instead of creating new strokes
            ...
            UIS.InputEnded:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 then
                    ...
                    trackStroke.Color=C.br0  -- restore on mouse up too
                end
            end)
            ...
            -- FIX 3: update existing trackStroke.Color — no new stroke() calls
        ...
        function CAT:AddKeybind(o5)
            ...
            UIS.InputBegan:Connect(function(i,gpe)
                if listening and not gpe and i.UserInputType==Enum.UserInputType.Keyboard then key=i.KeyCode;kb.Text=keyName(key);kb.TextColor3=AC;kb.BackgroundColor3=C.bg4;listening=false
                elseif not listening and not gpe and i.KeyCode==key and key~=Enum.KeyCode.Unknown then pcall(cb);if flag then _G[flag]=key end end
            end)
