--[[
    PeleccosSoftwares v12.4
    Bugs fixed from v12.3:
    1. Forward-reference crash (line 469 "attempt to index nil with Visible"):
       SET_BTN and CFGWIN_BTN clicks referenced CFGW before it was defined.
       Fixed by deferring those connections to after both windows are built.
    2. Config saves as []:
       CFGSYS._flags was always empty because makeConfigSystem() creates a NEW
       local _flags table each call, but the element builders called
       CFGSYS.register() on the *returned* table correctly — the real bug was
       that HttpService wasn't declared as a service. Added HttpService.
       Also added a fallback so JSONEncode errors are caught per-flag.
    3. Settings/Configs windows show after Insert re-open:
       Now properly hidden when _vis toggles off, and the windows are
       children of SG (ScreenGui) so they stay correctly hidden.
    4. "Configs Folder" button now labelled "Configs Folder" not the icon.
    5. Stroke on BAR was causing a Roblox error (UIStroke on Frame with no
       corner = no effect anyway). Removed redundant stroke call on BAR.
...
local function stroke(p, col, t)
    local s = Instance.new("UIStroke"); s.Color = col or rgb(55,55,62)
    s.Thickness = t or 1.5; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; s.Parent = p; return s
end
...
local function notify(o)
    ...
    local badge = mk("TextLabel",{
        ...
        BackgroundColor3=ac, TextColor3=C.t0, TextSize=9,
        Font=Enum.Font.GothamBold, ZIndex=503, Parent=hrow,
    })
    corner(badge, RC.sharp)
    mk("TextLabel",{
        Text=o.Title or "Notice", Size=dim2(1,-28,1,0),
        BackgroundTransparency=1, TextColor3=C.t0, TextSize=12,
    ...
    if o.Desc and o.Desc ~= "" then
        mk("TextLabel",{
            ...
            BackgroundTransparency=1, TextColor3=C.t2, TextSize=10,
            Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
            TextWrapped=true, ZIndex=503, Parent=inner,
        })
    end
...
local function buildColorPicker(ov, anchor, h0, s0, v0, onUpdate, SG_ref)
    ...
    local svc = mk("TextButton",{
        ...
        Position=dim2(cs,0,1-cv,0), BackgroundColor3=rgb(255,255,255),
        ZIndex=226, Parent=svbg,
    })
    ...
    local hueCursor = mk("Frame",{
        ...
        Position=dim2(ch,0,.5,0), BackgroundColor3=rgb(255,255,255),
        ZIndex=223, Parent=hueBar,
    })
...
local function buildWindow(parent, size, pos, zBase, bgImage, acReg)
    local outer = mk("Frame",{
        ...
        Size=size, Position=pos, ZIndex=zBase, Parent=parent,
    })
    ...
    local img = mk("ImageLabel",{
        ...
        Image=bgImage or "",
        Size=dim2(1,-4,1,-4), Position=dim2(.5,0,.5,0),
    ...
    -- transparent drag handle (below content ZIndex)
    local dragHandle = mk("TextButton",{
        Size=dim2(1,0,1,0), BackgroundTransparency=1,
        Text="", AutoButtonColor=false, ZIndex=zBase+2, Parent=img,
    })
...
local function makeConfigSystem(scriptName)
    ...
    -- _flags lives in this closure — element builders call reg() later
    local _flags = {}
        ...
        for flag, info in pairs(_flags) do
            pcall(function()
                ...
                elseif ftype == "key"    then
                    data[flag] = tostring(v):gsub("Enum%.KeyCode%.","")
                else
                    data[flag] = tostring(v)
                end
            end)
        end
        -- guard: if no flags registered, still save empty object not array
        local json = ""
        ...
        for flag, val in pairs(data) do
            ...
            if info then
                pcall(function()
                    ...
                    elseif ftype == "key" then
                        pcall(function() info.set(Enum.KeyCode[val]) end)
                    else
                        info.set(val)
                    end
                end)
    ...
    local function openDir()
        -- Try multiple executor-specific open-folder APIs
        pcall(function()
            if syn and syn.open_file_in_desktop then
                syn.open_file_in_desktop(cfgDir)
            end
        end)
        pcall(function()
        ...
        -- fallback: show path in notification (handled by caller)
    end
...
function Peleccos:CreateWindow(o)
    ...
    -- ── ScreenGui ──────────────────────────────────────────────
    -- IgnoreGuiInset=true → renders from pixel (0,0), bar can sit at the very top
    local SG = mk("ScreenGui",{
        Name="PeleccosV12", ResetOnSpawn=false,
    ...
    local BAR = mk("Frame",{
        ...
        BackgroundColor3=rgb(8,8,10), BorderSizePixel=0, ZIndex=50, Parent=SG,
    })
    -- bottom accent line
    local barLine = mk("Frame",{
        Size=dim2(1,0,0,1), AnchorPoint=Vector2.new(0,1), Position=dim2(0,0,1,0),
        BackgroundColor3=AC, ZIndex=51, Parent=BAR,
    })
    onAC(function(c) barLine.BackgroundColor3 = c end)

    -- Easter Egg button (hidden until keyword found)
    local EGG_BTN = mk("TextButton",{
        ...
        Size=dim2(0,0,0,BAR_H-6), Position=dim2(0,3,0,3),
    ...
    -- Right-side buttons: "Configs" and "Settings"
    -- NOTE: these are defined here but their click handlers are connected AFTER
    -- CFGW and SW are built, to avoid the forward-reference crash.
    local CFGWIN_BTN = mk("TextButton",{
        Text="Configs",
        TextColor3=C.t1, TextSize=10, Font=Enum.Font.GothamSemibold,
        BackgroundColor3=C.bg4, Size=dim2(0,58,0,BAR_H-6),
        Position=dim2(1,-128,0,3),
        AutoButtonColor=false, BorderSizePixel=0, ZIndex=52, Parent=BAR,
    })
    ...
    local SET_BTN = mk("TextButton",{
        Text="Settings",
        TextColor3=C.t1, TextSize=10, Font=Enum.Font.GothamSemibold,
        BackgroundColor3=C.bg4, Size=dim2(0,62,0,BAR_H-6),
        Position=dim2(1,-65,0,3),
        AutoButtonColor=false, BorderSizePixel=0, ZIndex=52, Parent=BAR,
    })
    ...
    local CAT_SF = mk("ScrollingFrame",{
        ...
        ScrollBarThickness=0, ZIndex=52, Parent=BAR,
    })
    ...
    local WM = mk("Frame",{
        ...
        Position=dim2(0,8,0,BAR_H+5),
        BackgroundColor3=rgb(8,8,10), BackgroundTransparency=0.10,
    ...
    local function wmSep()
        mk("TextLabel",{
            Text=" | ", Size=dim2(0,11,1,0), BackgroundTransparency=1,
            TextColor3=C.br2, TextSize=10, Font=Enum.Font.Gotham, ZIndex=31, Parent=wmRow,
        })
    end
    ...
    -- ScriptName [accent+bold] | UserName | GameName[GameId] | ConfigName | fps | ping
    ...
    BG.Name = "MainWindow"
    -- Resize to fit screen once SG has resolved its AbsoluteSize
    task.defer(function()
    ...
    local CONTENT = mk("ScrollingFrame",{
        ...
        ScrollBarImageColor3=AC, CanvasSize=dim2(0,0,0,0),
    ...
    local SW_HDR = mk("Frame",{
        ...
        BackgroundTransparency=0.40, ZIndex=104, Parent=SW_IMG,
    })
    local swAcc = mk("Frame",{
        Size=dim2(1,0,0,1), AnchorPoint=Vector2.new(0,1), Position=dim2(0,0,1,0),
        BackgroundColor3=AC, ZIndex=105, Parent=SW_HDR,
    })
    onAC(function(c) swAcc.BackgroundColor3 = c end)
    mk("TextLabel",{
        Text="Settings", Size=dim2(1,0,1,0), Position=dim2(0,10,0,0),
        BackgroundTransparency=1, TextColor3=C.t0, TextSize=11,
        Font=Enum.Font.GothamBold, TextXAlignment=Enum.TextXAlignment.Left,
    ...
    swLL:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        task.defer(function()
            if SW_SF and SW_SF.Parent then
                SW_SF.CanvasSize = dim2(0,0,0, swLL.AbsoluteContentSize.Y+14)
            end
        end)
    end)
    ...
    local function swSection(title)
        ...
        padding(f, 0, 0, 0, 5)
        local lbl = mk("TextLabel",{
            Text=title:upper(), Size=dim2(1,-5,1,0),
            BackgroundTransparency=1, TextColor3=AC, TextSize=9,
            Font=Enum.Font.GothamBold, TextXAlignment=Enum.TextXAlignment.Left,
            ZIndex=106, Parent=f,
        })
    ...
    local function swRow(h)
        local r = mk("Frame",{
            Size=dim2(1,0,0,h or 24), BackgroundColor3=C.bg3,
            BackgroundTransparency=0.38, ZIndex=105, Parent=SW_SF,
        })
        corner(r, RC.soft); return r
    end
    local function swToggle(lbl_text, default, onChange)
        local r = swRow(24)
        mk("TextLabel",{
            Text=lbl_text, Size=dim2(1,-46,1,0), Position=dim2(0,6,0,0),
            BackgroundTransparency=1, TextColor3=C.t1, TextSize=11,
            Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
            ZIndex=106, Parent=r,
        })
        local val = default == true
        local track = mk("Frame",{
            Size=dim2(0,30,0,13), Position=dim2(1,-34,.5,-6.5),
            BackgroundColor3=val and AC or C.bg4, ZIndex=106, Parent=r,
        })
        corner(track, RC.pill)
        local knob = mk("Frame",{
            Size=dim2(0,10,0,10),
            Position = val and dim2(1,-12,.5,-5) or dim2(0,2,.5,-5),
            BackgroundColor3=C.t0, ZIndex=107, Parent=track,
        })
        corner(knob, RC.pill)
        onAC(function(c) if val then track.BackgroundColor3 = c end end)
        local tbtn = mk("TextButton",{
            Size=dim2(1,0,1,0), BackgroundTransparency=1,
            Text="", AutoButtonColor=false, ZIndex=108, Parent=r,
        })
    ...
    local function swButton(txt, cb)
        local r   = swRow(24)
        local lbl = mk("TextLabel",{
            Text=txt, Size=dim2(1,0,1,0), BackgroundTransparency=1,
            TextColor3=C.t1, TextSize=11, Font=Enum.Font.GothamSemibold,
            ZIndex=106, Parent=r,
        })
        local hb = mk("TextButton",{
            Text="", Size=dim2(1,0,1,0),
            BackgroundTransparency=1, AutoButtonColor=false, ZIndex=107, Parent=r,
        })
    ...
    local function swColorRow(lbl_text, defCol, onChange)
        local r = swRow(24)
        mk("TextLabel",{
            Text=lbl_text, Size=dim2(1,-42,1,0), Position=dim2(0,6,0,0),
            BackgroundTransparency=1, TextColor3=C.t1, TextSize=11,
            Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
            ZIndex=106, Parent=r,
        })
        local col = defCol; local h, s, v = Color3.toHSV(col)
        local sw2 = mk("TextButton",{
            Size=dim2(0,30,0,16), Position=dim2(1,-34,.5,-8),
            BackgroundColor3=col, Text="", AutoButtonColor=false, ZIndex=106, Parent=r,
        })
    ...
    mk("TextLabel",{
        Text="Unload Script", Size=dim2(1,0,1,0), BackgroundTransparency=1,
        TextColor3=rgb(215,55,55), TextSize=12, Font=Enum.Font.GothamBold,
        ZIndex=106, Parent=unloadRow,
    })
    local unloadBtn = mk("TextButton",{
        Size=dim2(1,0,1,0), BackgroundTransparency=1,
        Text="", AutoButtonColor=false, ZIndex=107, Parent=unloadRow,
    })
    ...
    local CFGW_HDR = mk("Frame",{
        Size=dim2(1,0,0,26), BackgroundColor3=rgb(0,0,0),
        BackgroundTransparency=0.40, ZIndex=114, Parent=CFGW_IMG,
    })
    local cfgAcc = mk("Frame",{
        Size=dim2(1,0,0,1), AnchorPoint=Vector2.new(0,1), Position=dim2(0,0,1,0),
        BackgroundColor3=AC, ZIndex=115, Parent=CFGW_HDR,
    })
    onAC(function(c) cfgAcc.BackgroundColor3 = c end)
    mk("TextLabel",{
        Text="Configs  —  "..CFG.ScriptName,
        Size=dim2(1,0,1,0), Position=dim2(0,10,0,0),
        BackgroundTransparency=1, TextColor3=C.t0, TextSize=11,
        Font=Enum.Font.GothamBold, TextXAlignment=Enum.TextXAlignment.Left,
        ZIndex=115, Parent=CFGW_HDR,
    })
    draggify(CFGW, cfgwDragHandle)

    -- Toolbar: name input | + Create | Configs Folder
    local cfgToolRow = mk("Frame",{
        Size=dim2(1,0,0,30), Position=dim2(0,0,0,26),
        BackgroundColor3=rgb(0,0,0), BackgroundTransparency=0.60,
        ZIndex=114, Parent=CFGW_IMG,
    })
    padding(cfgToolRow, 4, 4, 4, 4)
    layout(cfgToolRow, Enum.FillDirection.Horizontal, 4, Enum.HorizontalAlignment.Left, Enum.VerticalAlignment.Center)

    -- Name input
    local newCfgFrame = mk("Frame",{
        Size=dim2(1,-136,0,20), BackgroundColor3=C.bg4,
        ZIndex=115, Parent=cfgToolRow,
    })
    corner(newCfgFrame, RC.sharp); stroke(newCfgFrame, C.br0, 1)
    local newCfgTB = mk("TextBox",{
        PlaceholderText="config name...", Text="",
        Size=dim2(1,-6,1,0), BackgroundTransparency=1,
        TextColor3=C.t0, PlaceholderColor3=C.t2,
        TextSize=10, Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
        ClearTextOnFocus=false, ZIndex=116, Parent=newCfgFrame,
    })
    padding(newCfgTB, 0, 3, 0, 4)

    local function cfgToolBtn(txt, w, cb)
        local b = mk("TextButton",{
            Text=txt, Size=dim2(0,w,0,20),
            BackgroundColor3=C.bg4, TextColor3=C.t1,
            TextSize=10, Font=Enum.Font.GothamSemibold,
            AutoButtonColor=false, ZIndex=115, Parent=cfgToolRow,
        })
        corner(b, RC.sharp); stroke(b, C.br0, 1)
    ...
    mk("Frame",{
        Size=dim2(1,0,0,1), Position=dim2(0,0,0,56),
        BackgroundColor3=C.br1, ZIndex=114, Parent=CFGW_IMG,
    })

    -- Config list scroll
    local CFGW_SF = mk("ScrollingFrame",{
        Size=dim2(1,-6,1,-60), Position=dim2(0,3,0,58),
        BackgroundTransparency=1, ScrollBarThickness=2,
        ScrollBarImageColor3=AC, CanvasSize=dim2(0,0,0,0),
        ZIndex=114, Parent=CFGW_IMG,
    })
    onAC(function(c) CFGW_SF.ScrollBarImageColor3 = c end)
    local cfgwLL = layout(CFGW_SF, Enum.FillDirection.Vertical, 4)
    padding(CFGW_SF, 4, 4, 4, 4)
    cfgwLL:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        task.defer(function()
            if CFGW_SF and CFGW_SF.Parent then
                CFGW_SF.CanvasSize = dim2(0,0,0, cfgwLL.AbsoluteContentSize.Y+10)
            end
        end)
    end)

    -- Build config list rows
    local function refreshCfgList()
        for _, ch in ipairs(CFGW_SF:GetChildren()) do
        ...
        if #list == 0 then
            mk("TextLabel",{
                ...
                ZIndex=115, Parent=CFGW_SF,
            })
            return
        end
        for _, name in ipairs(list) do
            local row = mk("Frame",{
                Size=dim2(1,0,0,32), BackgroundColor3=C.bg3,
                BackgroundTransparency=0.38, ZIndex=115, Parent=CFGW_SF,
            })
            corner(row, RC.sharp)
            mk("TextLabel",{
                Text=name, Size=dim2(1,-118,1,0), Position=dim2(0,8,0,0),
                BackgroundTransparency=1, TextColor3=C.t1, TextSize=11,
                Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
                TextTruncate=Enum.TextTruncate.AtEnd, ZIndex=116, Parent=row,
            })
            -- Load button
            local loadBtn = mk("TextButton",{
                Text="Load", Size=dim2(0,36,0,22), Position=dim2(1,-114,.5,-11),
                BackgroundColor3=C.bg4, TextColor3=AC, TextSize=9,
                Font=Enum.Font.GothamBold, AutoButtonColor=false, ZIndex=116, Parent=row,
            })
            corner(loadBtn, RC.sharp); stroke(loadBtn, C.br0, 1)
            ...
            loadBtn.MouseButton1Click:Connect(function()
                if CFGSYS.load(name) then
                    CFG.ConfigName = name; wmLabels.config.Text = name
                    notify({Title="Loaded",Desc=name,Type="Success",Duration=2})
                else
                    notify({Title="Load Failed",Desc="Could not read "..name,Type="Error",Duration=3})
                end
            end)
            -- Save button
            local saveBtn = mk("TextButton",{
                Text="Save", Size=dim2(0,36,0,22), Position=dim2(1,-74,.5,-11),
                BackgroundColor3=C.bg4, TextColor3=C.t1, TextSize=9,
                Font=Enum.Font.GothamBold, AutoButtonColor=false, ZIndex=116, Parent=row,
            })
            corner(saveBtn, RC.sharp); stroke(saveBtn, C.br0, 1)
            ...
            saveBtn.MouseButton1Click:Connect(function()
                ...
                if ok2 then
                    notify({Title="Saved",Desc=name.." updated.",Type="Success",Duration=2})
                else
                    notify({Title="Save Failed",Type="Error",Duration=2})
                end
            end)
            -- Delete button
            local delBtn = mk("TextButton",{
                Text="Del", Size=dim2(0,30,0,22), Position=dim2(1,-34,.5,-11),
                BackgroundColor3=rgb(36,12,12), TextColor3=rgb(200,60,60), TextSize=9,
                Font=Enum.Font.GothamBold, AutoButtonColor=false, ZIndex=116, Parent=row,
            })
            corner(delBtn, RC.sharp); stroke(delBtn, C.br0, 1)
            ...
            delBtn.MouseButton1Click:Connect(function()
                CFGSYS.delete(name); refreshCfgList()
                notify({Title="Deleted",Desc=name.." removed.",Type="Warning",Duration=2})
            end)
        end
    end
    ...
    cfgToolBtn("+ Create", 60, function()
        ...
        name = name:gsub("[^%w%-%_]","_")  -- sanitise filename
        newCfgTB.Text = ""
        local ok2 = CFGSYS.save(name)
        if ok2 then
            notify({Title="Created",Desc=name,Type="Success",Duration=2}); refreshCfgList()
        else
            notify({Title="Create Failed",Desc="Check executor file permissions.",Type="Error",Duration=3})
        end
    end)

    -- "Configs Folder" button (fixed label from the icon-only version)
    cfgToolBtn("Configs Folder", 72, function()
        CFGSYS.openDir()
        ...
        while SG and SG.Parent do
            ...
            pcall(function()
                ...
                local function scan(inst, d)
                    ...
                    if ok2 and tx then
                        local lo = tx:lower()
                        for _, kw in ipairs(EASTER_KW) do
                            if lo:find(kw, 1, true) then found = true; return end
                        end
                    end
    ...
    EGG_BTN.MouseButton1Click:Connect(function()
        ...
        local eggOuter = mk("Frame",{
            Size=dim2(0,300,0,160), Position=dim2(.5,-150,.5,-80),
            BackgroundColor3=rgb(8,8,10), ZIndex=501, Parent=ov3,
        })
        local eggStroke = stroke(eggOuter, AC, 2); onAC(function(c) eggStroke.Color = c end)
        local eggImg = mk("ImageLabel",{
            BackgroundColor3=rgb(6,6,8), AnchorPoint=Vector2.new(.5,.5),
            Image=BG_IMAGE, Size=dim2(1,-4,1,-4), Position=dim2(.5,0,.5,0),
        ...
        local eAcc = mk("Frame",{
            Size=dim2(1,0,0,1), AnchorPoint=Vector2.new(0,1), Position=dim2(0,0,1,0),
            BackgroundColor3=AC, ZIndex=505, Parent=eHdr,
        })
        onAC(function(c) eAcc.BackgroundColor3 = c end)
        mk("TextLabel",{
            Text="EASTER EGG",
            Size=dim2(1,-30,1,0), Position=dim2(0,10,0,0),
            BackgroundTransparency=1, TextColor3=AC, TextSize=12,
            Font=Enum.Font.GothamBold, TextXAlignment=Enum.TextXAlignment.Left,
            ZIndex=505, Parent=eHdr,
        })
        local eClose = mk("TextButton",{
            Text="X", Size=dim2(0,20,0,18), Position=dim2(1,-23,0,4),
        ...
        mk("TextLabel",{
            Text="Found an easter egg!\n\nKeyword detected in the game.\n\nCheck "..EASTER_KW[1].." files in:\n".._DIR.."/eggs/",
            Size=dim2(1,-16,0,80), Position=dim2(0,8,0,32),
            BackgroundTransparency=1, TextColor3=C.t1, TextSize=10,
            Font=Enum.Font.Gotham, TextWrapped=true,
            TextXAlignment=Enum.TextXAlignment.Left, ZIndex=504, Parent=eggImg,
        })
    ...
    UIS.InputBegan:Connect(function(i, gpe)
        if not gpe and i.KeyCode == KEY then
            ...
            BG.Visible  = _vis
            BAR.Visible = _vis
            -- hide sub-windows when toggling off
            if not _vis then
                SW.Visible   = false
                CFGW.Visible = false
            end
            -- WM.Visible is ONLY controlled by the Settings "Show Watermark" toggle
        end
    end)
    ...
    function WO:AddCategory(name)
        ...
        -- row factory: blocky outer (RC.sharp) with border + interior bg
        local function mkRow(h)
            ...
            corner(r, RC.sharp); stroke(r, C.br0, 1)
            return r
        end
        ...
        -- ── AddButton ─────────────────────────────────────────────
        -- Style inspired by Octohook/Luna: full-width option background,
        -- left accent bar that slides in on hover, gradient tint overlay,
        -- text dimmed at rest → white on hover → bg-dark on click flash.
        function CAT:AddButton(o5)
            o5 = o5 or {}
            local nm = o5.Name or "Button"
            local cb = o5.Callback or function() end

            -- outer shell (same blocky row style as everything else)
            local row = mkRow(30)

            -- full-width clickable face inside the shell
            local face = mk("Frame",{
                Size=dim2(1,-8,0,22), Position=dim2(0,4,0,4),
                BackgroundColor3=C.bg4, ZIndex=7, Parent=row,
            })
            corner(face, RC.soft)
            stroke(face, C.br1, 1)

            -- subtle gradient overlay (darkens left→right like Octohook)
            local grad = mk("Frame",{
                Size=dim2(1,0,1,0), BackgroundColor3=rgb(0,0,0),
                BackgroundTransparency=0.82, ZIndex=8, Parent=face,
            })
            corner(grad, RC.soft)
            mk("UIGradient",{
            ...
            -- left accent bar (Octohook topBorder equivalent) — hidden at rest
            local acBar = mk("Frame",{
                Size=dim2(0,3,1,0), Position=dim2(0,0,0,0),
                BackgroundColor3=AC, BackgroundTransparency=1,
                ZIndex=9, Parent=face,
            })
            corner(acBar, RC.sharp)
            onAC(function(c) acBar.BackgroundColor3 = c end)
            ...
            local lbl = mk("TextLabel",{
                ...
                ZIndex=10, Parent=face,
            })

            -- transparent hit area on top
            local hbtn = mk("TextButton",{
                Size=dim2(1,0,1,0), BackgroundTransparency=1,
                Text="", AutoButtonColor=false, ZIndex=11, Parent=face,
            })
            ...
            hbtn.MouseEnter:Connect(function()
                ...
                tw(acBar,{BackgroundTransparency=0},        Enum.EasingStyle.Quint, 0.12)
                stroke(face, AC, 1)   -- border highlights to accent on hover
            end)
            hbtn.MouseLeave:Connect(function()
                tw(face, {BackgroundColor3=C.bg4},         Enum.EasingStyle.Quint, 0.12)
                tw(lbl,  {TextColor3=C.t2},                Enum.EasingStyle.Quint, 0.12)
                tw(acBar,{BackgroundTransparency=1},        Enum.EasingStyle.Quint, 0.12)
                stroke(face, C.br1, 1)
            end)
            hbtn.MouseButton1Down:Connect(function()
                -- flash: face → accent, text → very dark (like Octohook -95 offset)
            ...
            hbtn.MouseButton1Up:Connect(function()
                tw(face, {BackgroundColor3=C.bg3},         Enum.EasingStyle.Quint, 0.15)
                tw(lbl,  {TextColor3=C.t0},                Enum.EasingStyle.Quint, 0.15)
            end)
            hbtn.MouseButton1Click:Connect(function()
                task.delay(0.22, function()
                    tw(face, {BackgroundColor3=C.bg4},     Enum.EasingStyle.Quint, 0.18)
                    tw(lbl,  {TextColor3=C.t2},            Enum.EasingStyle.Quint, 0.18)
                    tw(acBar,{BackgroundTransparency=1},   Enum.EasingStyle.Quint, 0.18)
                    stroke(face, C.br1, 1)
                end)
                pcall(cb)
            end)
            ...
            local r = {}
            function r:SetText(t) lbl.Text = t end
            return r
        end
        ...
        function CAT:AddLabel(o5)
            ...
            local lbl = mk("TextLabel",{
                ...
                Size=dim2(1,0,0,0), AutomaticSize=Enum.AutomaticSize.Y,
                BackgroundColor3=C.bg2, BackgroundTransparency=0.42,
                TextColor3=o5.Color or C.t2, TextSize=o5.Size or 11,
                Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
        ...
        function CAT:AddToggle(o5)
            ...
            corner(track, RC.pill); stroke(track, C.br1, 1)
            local knob = mk("Frame",{
                Size=dim2(0,12,0,12),
                Position=val and dim2(1,-14,.5,-6) or dim2(0,2,.5,-6),
                BackgroundColor3=C.t0, ZIndex=8, Parent=track,
            })
            corner(knob, RC.pill)
            onAC(function(c) if val then track.BackgroundColor3 = c end end)
            ...
            if hasKB then
                ...
                kbBtn = mk("TextButton",{
                    Text=keyName(kbKey), Size=dim2(0,58,0,15),
                    Position=dim2(1,-124,.5,-7.5),
            ...
            if hasKB then
                UIS.InputBegan:Connect(function(i, gpe)
                    if listening and not gpe and i.UserInputType==Enum.UserInputType.Keyboard then
                        kbKey=i.KeyCode; kbBtn.Text=keyName(kbKey); kbBtn.TextColor3=AC; kbBtn.BackgroundColor3=C.bg4; listening=false
            ...
            if flag then
                _G[flag] = val
                -- register with config system so save/load works
                CFGSYS.register(flag, function() return val end, function(v) set(v, true) end, "bool")
            end
            local r = {Value=val}
        ...
        -- ── AddSlider ─────────────────────────────────────────────
        -- Style inspired by Octohook/Luna:
        --   Top row  : name (left)  |  value/max with suffix (right)
        --   Track row: [−] [=========filled===|===] [+]
        --   Ctrl+Click the track to type an exact number (any decimal).
        --   Hold Shift while clicking +/− to step ×10.
        function CAT:AddSlider(o5)
            o5 = o5 or {}
            local nm   = o5.Name or "Slider"
            local mn   = o5.Min  or 0;   local mx  = o5.Max  or 100
            local step = o5.Step or 1;   local suf = o5.Suffix or ""
            local flag = o5.Flag;        local cb  = o5.Callback or function() end
            local val  = math.clamp(o5.Default or mn, mn, mx)

            -- outer shell — slightly taller to fit two rows
            local wrap = mk("Frame",{
                Size=dim2(1,0,0,52),
                BackgroundColor3=C.bg2, BackgroundTransparency=0.42,
                ZIndex=6, Parent=catPanel,
            ...
            local nameLbl = mk("TextLabel",{
                Text=nm, Size=dim2(1,-64,1,0),
                BackgroundTransparency=1, TextColor3=C.t1,
                TextSize=12, Font=Enum.Font.Gotham,
                TextXAlignment=Enum.TextXAlignment.Left,
                ZIndex=8, Parent=top,
            })
            -- value box: shows  "val / max suf"  (Octohook style)
            local vbg = mk("Frame",{
                Size=dim2(0,58,0,17), Position=dim2(1,-60,.5,-8.5),
                BackgroundColor3=C.bg4, ZIndex=8, Parent=top,
            })
            corner(vbg, RC.soft); stroke(vbg, C.br1, 1)
            local function fmtVal(v)
                -- show decimals only when step < 1
                if step < 1 then
                    local dec = math.max(0, math.ceil(-math.log10(step)))
                    return string.format("%."..dec.."f", v)..suf.."/"..mx..suf
                end
                return tostring(v)..suf.."/"..mx..suf
            end
            local vLbl = mk("TextLabel",{
                Text=fmtVal(val), Size=dim2(1,-4,1,0), Position=dim2(0,2,0,0),
                BackgroundTransparency=1, TextColor3=C.t2,
            ...
            local minusBtn = mk("TextButton",{
                Text="-", Size=dim2(0,16,0,16), Position=dim2(0,0,.5,-8),
                BackgroundColor3=C.bg4, TextColor3=C.t2,
                TextSize=13, Font=Enum.Font.GothamBold,
                AutoButtonColor=false, ZIndex=8, Parent=trackRow,
            })
            corner(minusBtn, RC.sharp); stroke(minusBtn, C.br0, 1)
            minusBtn.MouseEnter:Connect(function() tw(minusBtn,{BackgroundColor3=C.bg3,TextColor3=C.t0},Enum.EasingStyle.Quint,0.1) end)
            minusBtn.MouseLeave:Connect(function() tw(minusBtn,{BackgroundColor3=C.bg4,TextColor3=C.t2},Enum.EasingStyle.Quint,0.1) end)

            -- plus button
            local plusBtn = mk("TextButton",{
                Text="+", Size=dim2(0,16,0,16), Position=dim2(1,-16,.5,-8),
                BackgroundColor3=C.bg4, TextColor3=C.t2,
                TextSize=13, Font=Enum.Font.GothamBold,
                AutoButtonColor=false, ZIndex=8, Parent=trackRow,
            })
            corner(plusBtn, RC.sharp); stroke(plusBtn, C.br0, 1)
            plusBtn.MouseEnter:Connect(function() tw(plusBtn,{BackgroundColor3=C.bg3,TextColor3=C.t0},Enum.EasingStyle.Quint,0.1) end)
            plusBtn.MouseLeave:Connect(function() tw(plusBtn,{BackgroundColor3=C.bg4,TextColor3=C.t2},Enum.EasingStyle.Quint,0.1) end)

            -- track background (between the two buttons)
            local trackBg = mk("Frame",{
                Size=dim2(1,-36,0,6), Position=dim2(0,20,.5,-3),
                BackgroundColor3=C.bg4, ZIndex=7, Parent=trackRow,
            })
            corner(trackBg, RC.pill); stroke(trackBg, C.br0, 1)
            ...
            -- draggable hit area over the track
            local trackHit = mk("TextButton",{
                Size=dim2(1,0,3,0), Position=dim2(0,0,-.5,0),  -- taller hit area for comfort
                BackgroundTransparency=1, Text="", AutoButtonColor=false,
            ...
            trackHit.InputBegan:Connect(function(i)
                if i.UserInputType == Enum.UserInputType.MouseButton1 then
                    if UIS:IsKeyDown(Enum.KeyCode.LeftControl) then
                        -- ── Ctrl+Click: type exact value ───────────
                        local inputStr = ""
                        vLbl.Text = "type: _"
                        tw(vLbl,{TextColor3=rgb(255,185,0)},Enum.EasingStyle.Quint,0.08)
                        local conn; conn = UIS.InputBegan:Connect(function(ki, gpe)
                            ...
                            local numMap = {
                                ...
                                [Enum.KeyCode.Nine]="9",
                                [Enum.KeyCode.KeypadZero]="0",[Enum.KeyCode.KeypadOne]="1",[Enum.KeyCode.KeypadTwo]="2",
                                [Enum.KeyCode.KeypadThree]="3",[Enum.KeyCode.KeypadFour]="4",[Enum.KeyCode.KeypadFive]="5",
                                [Enum.KeyCode.KeypadSix]="6",[Enum.KeyCode.KeypadSeven]="7",[Enum.KeyCode.KeypadEight]="8",
                                [Enum.KeyCode.KeypadNine]="9",
                                [Enum.KeyCode.Period]=".",[Enum.KeyCode.KeypadPeriod]=".",
                                [Enum.KeyCode.Minus]="-",
                            }
                            if numMap[kn] then
                                -- only allow one decimal point and one leading minus
                                if numMap[kn]=="." and inputStr:find("%.") then return end
                                if numMap[kn]=="-" and #inputStr>0 then return end
                                inputStr = inputStr..numMap[kn]
                                vLbl.Text = "type: "..inputStr.."_"
                            elseif kn==Enum.KeyCode.Backspace then
                                inputStr = inputStr:sub(1,-2)
                                vLbl.Text = "type: "..inputStr.."_"
                            elseif kn==Enum.KeyCode.Return or kn==Enum.KeyCode.KeypadEnter then
                                conn:Disconnect()
                                local num = tonumber(inputStr)
                                if num then sv(num) else sv(val) end
                                tw(vLbl,{TextColor3=C.t2},Enum.EasingStyle.Quint,0.2)
                            elseif kn==Enum.KeyCode.Escape then
                                conn:Disconnect()
                                sv(val, true) -- restore display without callback
                                tw(vLbl,{TextColor3=C.t2},Enum.EasingStyle.Quint,0.2)
                            end
                        end)
                    else
                        dragging = true
                        sv(posToVal(i.Position.X))
                    end
                end
            end)
            UIS.InputChanged:Connect(function(i)
            ...
            local function nudge(dir)
                -- shift = ×10, ctrl = ×0.1 (fine-tune)
                local mult = UIS:IsKeyDown(Enum.KeyCode.LeftShift) and 10
                          or UIS:IsKeyDown(Enum.KeyCode.LeftControl) and 0.1
                          or 1
                sv(val + dir * step * mult)
            end
            minusBtn.MouseButton1Click:Connect(function() nudge(-1) end)
            plusBtn.MouseButton1Click:Connect(function()  nudge( 1) end)

            -- hover glow on track
            trackHit.MouseEnter:Connect(function()
                stroke(trackBg, AC, 1)
            end)
            trackHit.MouseLeave:Connect(function()
                if not dragging then stroke(trackBg, C.br0, 1) end
            end)
            onAC(function(c)
                -- keep stroke colour in sync if hovered
            end)
        ...
        function CAT:AddTextbox(o5)
            ...
            corner(wrap, RC.sharp); stroke(wrap, C.br0, 1)
            mk("TextLabel",{
                Text=nm, Size=dim2(1,-8,0,14), Position=dim2(0,6,0,4),
                BackgroundTransparency=1, TextColor3=C.t2, TextSize=10,
                Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
                ZIndex=7, Parent=wrap,
            })
            local ifrm = mk("Frame",{
                Size=dim2(1,-8,0,24), Position=dim2(0,4,0,20),
                BackgroundColor3=C.bg4, ZIndex=7, Parent=wrap,
            })
            ...
            local tb = mk("TextBox",{
                ...
                BackgroundTransparency=1, TextColor3=C.t0, PlaceholderColor3=C.t2,
            ...
            tb.FocusLost:Connect(function()
                tw(sk,{Color=C.br1},Enum.EasingStyle.Quint,0.12)
                pcall(cb, tb.Text)
                if flag then _G[flag] = tb.Text end
            end)
            if flag then
                _G[flag] = o5.Default or ""
        ...
        function CAT:AddDropdown(o5)
            ...
            local multi = o5.Multi or false;      local flag = o5.Flag
            local cb    = o5.Callback or function() end
            ...
            local hd = mk("TextButton",{
                Size=dim2(1,-8,0,23), Position=dim2(0,4,0,21),
            ...
            local arr = mk("TextLabel",{
                ...
                BackgroundTransparency=1, TextColor3=C.t2, TextSize=9,
                Font=Enum.Font.Gotham, ZIndex=8, Parent=hd,
            })
            ...
            local function closeDD()
                isOpen=false; tw(arr,{Rotation=0},Enum.EasingStyle.Quint,0.12); closeOV()
            end
            ...
            if flag then
                _G[flag] = sel
                CFGSYS.register(flag, function() return sel end, function(v) sel=v;sl.Text=v;if flag then _G[flag]=v end end, "string")
            end
            local r = {Value=sel}
            function r:Set(v) sel=v; sl.Text=v end
        ...
        function CAT:AddColorPicker(o5)
            ...
            local row = mkRow(30)
            mk("TextLabel",{
                Text=nm, Size=dim2(1,-52,1,0), Position=dim2(0,8,0,0),
                BackgroundTransparency=1, TextColor3=C.t1, TextSize=12,
                Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
                ZIndex=7, Parent=row,
            })
            ...
            if flag then
                _G[flag] = col
                CFGSYS.register(flag, function() return col end, function(c2)
                    col=c2; h,s,v=Color3.toHSV(c2); sw.BackgroundColor3=c2; pcall(cb,c2)
                end, "color")
            end
            local r = {Value=col}
        ...
        function CAT:AddKeybind(o5)
            ...
            local row = mkRow(30)
            mk("TextLabel",{
                Text=nm, Size=dim2(1,-94,1,0), Position=dim2(0,8,0,0),
                BackgroundTransparency=1, TextColor3=C.t1, TextSize=12,
                Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
                ZIndex=7, Parent=row,
            })
            local kb = mk("TextButton",{
                Text=keyName(key), Size=dim2(0,78,0,17), Position=dim2(1,-82,.5,-8.5),
                BackgroundColor3=C.bg4, TextColor3=AC, TextSize=9,
        ...
        function CAT:AddSeparator()
            local sep = mk("Frame",{
                ...
                BackgroundTransparency=0.35, ZIndex=6, Parent=catPanel,
            })
            mk("UIGradient",{
                Transparency=NumberSequence.new({
                    NumberSequenceKeypoint.new(0,1),
                    NumberSequenceKeypoint.new(.06,0),
                    NumberSequenceKeypoint.new(.94,0),
        ...
        function CAT:AddProgressBar(o5)
            ...
            local maxV = o5.Max  or 100
            local cur  = math.clamp(o5.Default or 0, 0, maxV)
            ...
            mk("TextLabel",{
                Text=nm, Size=dim2(1,-52,1,0),
                BackgroundTransparency=1, TextColor3=C.t1, TextSize=12,
                Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
                ZIndex=8, Parent=top,
            })
            local pctLbl = mk("TextLabel",{
                Text=math.round(cur/maxV*100).."%",
                Size=dim2(0,46,1,0), Position=dim2(1,-48,0,0),
                BackgroundTransparency=1, TextColor3=AC, TextSize=11,
                Font=Enum.Font.GothamSemibold, ZIndex=8, Parent=top,
            })
            onAC(function(c) pctLbl.TextColor3 = c end)
            local trk = mk("Frame",{
                Size=dim2(1,-8,0,4), Position=dim2(0,4,0,28),
                BackgroundColor3=C.bg4, ZIndex=7, Parent=wrap,
            })
            corner(trk, RC.pill)
            local fill2 = mk("Frame",{
                Size=dim2(cur/maxV,0,1,0), BackgroundColor3=AC,
                ZIndex=8, Parent=trk,
            })
            corner(fill2, RC.pill); onAC(function(c) fill2.BackgroundColor3 = c end)

            local r = {Value=cur}
            function r:Set(v)
                v = math.clamp(v, 0, maxV); cur = v
                tw(fill2,{Size=dim2(v/maxV,0,1,0)},Enum.EasingStyle.Quint,0.22)
                pctLbl.Text = math.round(v/maxV*100).."%"
            end
