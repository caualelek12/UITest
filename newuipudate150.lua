...
function Peleccos:CreateWindow(o)
    ...
    function WO:AddCategory(name, _pinned)
        ...
        mk("Frame",{Name="Ind",Size=dim2(0,2,0,14),Position=dim2(0,0,0.5,-7),
        ...
        function CAT:_AddToggle(o5, parentOverride)
            ...
            local Lbl = mk("TextLabel",{Size=dim2(1,-22,1,0),BackgroundTransparency=1,Text=nm,
                TextColor3=val and C.Tx0 or C.Tx1,TextSize=11,Font=MenuFont,
                TextXAlignment=Enum.TextXAlignment.Left,Parent=row})
            onFont(function(mf,_) Lbl.Font=mf end)

            -- Checkbox: outer border box
            local BoxOuter = mk("Frame",{Size=dim2(0,16,0,16),Position=dim2(1,-18,0.5,-8),
                BackgroundColor3=val and AC or C.Toggle,BorderSizePixel=0,Parent=row})
            -- Inner fill (1px inset, only visible when off = shows bg)
            local BoxInner = mk("Frame",{Size=dim2(1,-2,1,-2),Position=dim2(0,1,0,1),
                BackgroundColor3=val and AC or C.BgRow,BorderSizePixel=0,Parent=BoxOuter})
            -- Checkmark label
            local Check = mk("TextLabel",{Size=dim2(1,0,1,0),BackgroundTransparency=1,
                Text=val and "X" or "",TextColor3=C.White,TextSize=10,
                Font=Enum.Font.GothamBold,Parent=BoxInner})
            onAC(function(c)
                if val then BoxOuter.BackgroundColor3=c; BoxInner.BackgroundColor3=c end
            end)

            local function set(v, silent)
                val=v
                local col = v and AC or C.Toggle
                tw(BoxOuter,{BackgroundColor3=col},0.08)
                tw(BoxInner,{BackgroundColor3=v and AC or C.BgRow},0.08)
    ...
    local function buildSettings()
        ...
        local function stToggle(lbl, default, onChange)
            ...
            mk("TextLabel",{Size=dim2(1,-22,1,0),BackgroundTransparency=1,Text=lbl,
                TextColor3=C.Tx0,TextSize=10,Font=MenuFont,
                TextXAlignment=Enum.TextXAlignment.Left,Parent=row})
            local val = default
            local BoxOuter = mk("Frame",{Size=dim2(0,14,0,14),Position=dim2(1,-16,0.5,-7),
                BackgroundColor3=val and AC or C.Toggle,BorderSizePixel=0,Parent=row})
            local BoxInner = mk("Frame",{Size=dim2(1,-2,1,-2),Position=dim2(0,1,0,1),
                BackgroundColor3=val and AC or C.BgRow,BorderSizePixel=0,Parent=BoxOuter})
            local Check = mk("TextLabel",{Size=dim2(1,0,1,0),BackgroundTransparency=1,
                Text=val and "X" or "",TextColor3=C.White,TextSize=9,
                Font=Enum.Font.GothamBold,Parent=BoxInner})
            onAC(function(c)
                if val then BoxOuter.BackgroundColor3=c; BoxInner.BackgroundColor3=c end
            end)
            ...
            Hit.MouseButton1Click:Connect(function()
                val=not val
                tw(BoxOuter,{BackgroundColor3=val and AC or C.Toggle},0.08)
                tw(BoxInner,{BackgroundColor3=val and AC or C.BgRow},0.08)
                Check.Text = val and "X" or ""
                pcall(onChange,val)
            end)
        end
    ...
    local function buildPlayerList()
        ...
        -- Platform guess from UserInputType (only meaningful for local player;
        -- for remote players we show "Unknown" since Roblox doesn't expose it)
        local function guessDevice(p)
            if p == LP then
                local last = UIS:GetLastInputType()
                if last == Enum.UserInputType.Touch then return "Mobile"
                elseif last == Enum.UserInputType.Gamepad1
                    or last == Enum.UserInputType.Gamepad2 then return "Console"
                else return "PC" end
            end
            return "Unknown"
        end
        ...
        local function buildCard(p)
            ...
            local infoFr = mk("Frame",{Size=dim2(1,0,0,52),BackgroundTransparency=1,Parent=expPanel})
            ...
            local device   = guessDevice(p)
            infoLine(3,  "Username:",   "@"..p.Name)
            infoLine(16, "Acct Age:",   ageDays)
            infoLine(29, "Premium:",    premium)
            infoLine(42, "Device:",     device)

            -- Separator
            mk("Frame",{Size=dim2(1,-10,0,1),Position=dim2(0,5,0,55),
                BackgroundColor3=C.BgGroupH,BorderSizePixel=0,Parent=expPanel})

            -- Action buttons row
            local actFr = mk("Frame",{Size=dim2(1,0,0,26),Position=dim2(0,0,0,58),
