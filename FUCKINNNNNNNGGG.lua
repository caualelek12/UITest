...
-- FOLDERS
-- ═══════════════════════════════════════════════════════════════
-- Root folder is created per-script in makeConfigSystem
...
local function addStroke(lbl, thickness, alpha)
...
-- Returns dark or light color for text/icons ON an accent-colored background
local function acContrast(ac)
    local lum = 0.299*(ac.R) + 0.587*(ac.G) + 0.114*(ac.B)
    return lum > 0.55 and Color3.fromRGB(20,20,20) or Color3.fromRGB(255,255,255)
end
local function contrastColor(c)
    local lum = 0.299*c.R + 0.587*c.G + 0.114*c.B
    return lum > 0.55 and rgb(12,14,17) or rgb(255,255,255)
end
...
local function makeConfigSystem(scriptName)
    local cfgDir = scriptName .. "/configs/"
    pcall(function()
    ...
    local function openDir()
        local opened = false
        pcall(function() if syn and syn.open_file_in_desktop then syn.open_file_in_desktop(cfgDir); opened=true end end)
        if not opened then pcall(function()
            if KRNL_ENV and KRNL_ENV.open_file_in_desktop then KRNL_ENV.open_file_in_desktop(cfgDir); opened=true end
        end) end
        if not opened then pcall(function()
            if typeof(open_file_in_desktop)=="function" then open_file_in_desktop(cfgDir); opened=true end
        end) end
        if not opened then pcall(function()
            if typeof(openFile)=="function" then openFile(cfgDir); opened=true end
        end) end
        if not opened then pcall(function()
            if typeof(open_file)=="function" then open_file(cfgDir); opened=true end
        end) end
        -- Fallback: copy path to clipboard so user can navigate manually
        if not opened then
            pcall(function() if typeof(setclipboard)=="function" then setclipboard(cfgDir) end end)
            pcall(function() if typeof(toclipboard)=="function" then toclipboard(cfgDir) end end)
        end
    end
    return {
...
function Peleccos:CreateWindow(o)
    ...
    local _vis           = true
    local _plUpdateDelay = 0.4   -- player list / distance refresh interval (seconds)

    -- Executor detection
    local _executor = "Unknown"
    pcall(function()
        if identifyexecutor then _executor = identifyexecutor()
        elseif syn          then _executor = "Synapse"
        elseif KRNL_ENV     then _executor = "Krnl"
        elseif getgenv and getgenv().__OXYGEN then _executor = "Oxygen"
        end
    end)
    ...
    if CFG.LogoImage ~= "" then
        local LogoImg = mk("ImageLabel", {
            ...
            ImageColor3=AC, Parent=LogoBox,
        })
        onAC(function(c) LogoImg.ImageColor3=c end)
    else
    ...
    local TabScroll = mk("ScrollingFrame", {
        Size=dim2(1,0,1,-102), Position=dim2(0,0,0,58),
    ...
    -- PinnedScroll removed — pinned tabs now live in TabScroll with high LayoutOrder
    local PinnedScroll = TabScroll  -- alias so AddCategory's tabParent assignment still works

    -- Player card (bottom of sidebar)
    mk("Frame", {
        Size=dim2(1,-16,0,1), Position=dim2(0,8,1,-102),
        BackgroundColor3=C.BgGroupH, BorderSizePixel=0, Parent=Sidebar,
    })
    local PCard = mk("Frame", {
        Size=dim2(1,0,0,102), Position=dim2(0,0,1,-102),
    ...
    local PDispName = mk("TextLabel", {
        Size=dim2(0,132,0,15), Position=dim2(0,44,0,10),
    ...
    local PUserName = mk("TextLabel", {
        Size=dim2(0,132,0,12), Position=dim2(0,44,0,27),
        BackgroundTransparency=1, Text="@"..LP.Name,
        TextColor3=C.Tx1, TextSize=10, Font=Enum.Font.GothamBold,
        TextXAlignment=Enum.TextXAlignment.Left,
        TextTruncate=Enum.TextTruncate.AtEnd, Parent=PCard,
    })
    -- Executor label
    mk("TextLabel", {
        Size=dim2(1,-16,0,13), Position=dim2(0,8,0,46),
        BackgroundTransparency=1, Text=_executor,
        TextColor3=C.Tx1, TextSize=11, Font=Enum.Font.GothamBold,
        TextXAlignment=Enum.TextXAlignment.Left, Parent=PCard,
    })
    -- Key time label (updatable via WO:SetKeyTime)
    local _keyTime = "∞"
    local wKeyTime = mk("TextLabel", {
        Size=dim2(1,-16,0,13), Position=dim2(0,8,0,62),
        BackgroundTransparency=1, Text="Key Time : ".._keyTime,
        TextColor3=C.Tx1, TextSize=11, Font=Enum.Font.GothamBold,
        TextXAlignment=Enum.TextXAlignment.Left, Parent=PCard,
    })
    RunService.Heartbeat:Connect(function()
    ...
    -- Key toggle visibility (animated)
    UIS.InputBegan:Connect(function(i, gpe)
        if not gpe and i.KeyCode == KEY then
            if _vis then
                tw(Main, {Size=dim2(0,30,0,20)}, 0.18, Enum.EasingStyle.Quint)
                task.delay(0.19, function()
                    _vis = false; Main.Visible = false
                    Main.Size = dim2(0,WIN_W,0,WIN_H)
                end)
            else
                _vis = true; Main.Visible = true
                Main.Position = dim2(0.5,-WIN_W/2,0.5,-WIN_H/2)
                Main.Size = dim2(0,30,0,20)
                tw(Main, {Size=dim2(0,WIN_W,0,WIN_H)}, 0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
            end
        end
    end)
    ...
    -- ─────────────────────────────────────────────────────────
    local function openColorPicker(getCol, onChange)
        openOV(function(ov)
            local ch2, cs2, cv2 = Color3.toHSV(getCol())
            local pw, ph = 210, 148
            local mp = UIS:GetMouseLocation()
            local px = math.min(mp.X + 6, SG.AbsoluteSize.X - pw - 10)
            local py = mp.Y + 6
            if py + ph > SG.AbsoluteSize.Y - 10 then py = mp.Y - ph - 6 end

            local pan = mk("Frame",{Size=dim2(0,pw,0,ph),Position=dim2(0,px,0,py),
                BackgroundColor3=C.BgSideBd,BorderSizePixel=0,ZIndex=220,Parent=ov})
            mk("UICorner",{CornerRadius=dim(0,6),Parent=pan})
            mk("UIStroke",{Color=rgb(44,52,60),Thickness=1,Parent=pan})
    ...
    local function openContextPopup(opts)
        openOV(function(ov)
            ...
            local Pop = mk("Frame",{Size=dim2(0,pw,0,ph),Position=dim2(0,px,0,py),
                BackgroundColor3=rgb(14,17,20),BorderSizePixel=0,ZIndex=250,Parent=ov})
            mk("UICorner",{CornerRadius=dim(0,4),Parent=Pop})
            mk("UIStroke",{Color=rgb(44,50,58),Thickness=1,Parent=Pop})

            -- Mode selector row
            local ModeRow = mk("Frame",{Size=dim2(1,-12,0,32),Position=dim2(0,6,0,8),
                BackgroundColor3=rgb(20,24,28),BorderSizePixel=0,ZIndex=251,Parent=Pop})
            mk("UICorner",{CornerRadius=dim(0,3),Parent=ModeRow})
            ...
            local function refreshModes()
                for _, info in ipairs(modeBtns) do
                    ...
                    info.btn.BackgroundColor3 = active and AC or rgb(20,24,28)
                    info.btn.TextColor3       = active and contrastColor(AC) or C.Tx0
                end
            end
            for mi, mOpt in ipairs(MODES) do
                local MB = mk("TextButton",{Size=dim2(1/3,0,1,0),BackgroundColor3=rgb(20,24,28),
                    Text=mOpt,TextColor3=C.Tx0,TextSize=11,Font=Enum.Font.GothamBold,
            ...
            refreshModes()
            onAC(function(_) refreshModes() end)

            -- Keybind row
            local KbRow = mk("Frame",{Size=dim2(1,-12,0,28),Position=dim2(0,6,0,48),
                BackgroundTransparency=1,ZIndex=251,Parent=Pop})
            local kbLbl = mk("TextLabel",{Size=dim2(0,56,1,0),BackgroundTransparency=1,Text="Keybind",
                TextColor3=C.Tx0,TextSize=11,Font=Enum.Font.GothamBold,
                TextXAlignment=Enum.TextXAlignment.Left,ZIndex=252,Parent=KbRow})
            addStroke(kbLbl)
            local kbBtn = mk("TextButton",{Size=dim2(1,-62,1,0),Position=dim2(0,60,0,0),
                BackgroundColor3=rgb(20,24,28),Text=keyName(opts.getKey()),
                TextColor3=AC,TextSize=11,Font=Enum.Font.GothamBold,
                AutoButtonColor=false,BorderSizePixel=0,ZIndex=252,Parent=KbRow})
            mk("UICorner",{CornerRadius=dim(0,3),Parent=kbBtn})
            addStroke(kbBtn)
    ...
    function WO:Toggle()
        if _vis then
            tw(Main,{Size=dim2(0,30,0,20)},0.18,Enum.EasingStyle.Quint)
            task.delay(0.19,function() _vis=false; Main.Visible=false; Main.Size=dim2(0,WIN_W,0,WIN_H) end)
        else
            _vis=true; Main.Visible=true
            Main.Position=dim2(0.5,-WIN_W/2,0.5,-WIN_H/2)
            Main.Size=dim2(0,30,0,20)
    ...
    function WO:SetKeyTime(t)
        _keyTime = tostring(t)
        if wKeyTime then wKeyTime.Text = "Key Time : ".._keyTime end
    end
    function WO:GetKeyTime()
        return _keyTime
    end
    ...
    function WO:AddCategory(name, _pinned)
        ...
        -- Sidebar button — ALL tabs go into TabScroll, pinned just get high LayoutOrder
        ...
        if _pinned then
            _pinnedOrder = _pinnedOrder + 1
            Btn.LayoutOrder = 9990 + _pinnedOrder
        else
            _dynOrder = _dynOrder + 1
            Btn.LayoutOrder = _dynOrder
        end
        ...
        local Lbl = mk("TextLabel",{Name="Lbl",Size=dim2(1,-16,1,0),Position=dim2(0,12,0,0),
            ...
            TextSize=13,Font=MenuFont,TextXAlignment=Enum.TextXAlignment.Left,Parent=Btn})
        ...
        local function addSubTab(label)
            ...
            local StLbl = mk("TextLabel",{Name="Lbl",Size=dim2(1,0,1,0),
                ...
                TextSize=11,Font=MenuFont,Parent=StBtn})
        ...
        local function mkGroup(parent, title)
            ...
            local HdrL = mk("TextLabel",{Size=dim2(1,-14,1,0),Position=dim2(0,6,0,0),
                ...
                TextSize=11,Font=MenuFont,TextXAlignment=Enum.TextXAlignment.Left,Parent=Hdr})
        ...
        function CAT:_AddToggle(o5, parentOverride)
            ...
            local Lbl = mk("TextLabel",{Size=dim2(1,-20,1,0),BackgroundTransparency=1,Text=nm,
                TextColor3=val and C.Tx0 or C.Tx2,TextSize=14,Font=MenuFont,
            ...
            onAC(function(c) if val then Box.BackgroundColor3=c; Lbl.TextColor3=c end end)

            local function set(v, silent)
                val=v
                tw(Box,{BackgroundColor3=v and AC or C.Toggle},0.1)
                Lbl.TextColor3 = v and AC or C.Tx2
            ...
            -- Right-click badge (shown only on keybind-enabled toggles)
            if allowKeybind then
                local rmbFr = mk("Frame",{
                    Size=dim2(0,30,0,13), Position=dim2(1,-48,0.5,-6),
                    BackgroundColor3=C.BgRow, BorderSizePixel=0, Parent=row,
                })
                mk("UICorner",{CornerRadius=dim(0,2),Parent=rmbFr})
                mk("TextLabel",{
                    Size=dim2(1,0,1,0), BackgroundTransparency=1, Text="RMB",
                    TextColor3=C.TxSub, TextSize=8, Font=Enum.Font.GothamBold,
                    Parent=rmbFr,
                })
            end
        ...
        function CAT:_AddSlider(o5, parentOverride)
            ...
            local NmLbl = mk("TextLabel",{Size=dim2(0.62,0,1,0),BackgroundTransparency=1,Text=nm,
                TextColor3=C.Tx0,TextSize=14,Font=MenuFont,
            ...
            -- End cap: vertical bar right at track's right edge (trackRow sibling)
            mk("Frame",{Size=dim2(0,3,0,13),Position=dim2(1,-43,0.5,-6),
                BackgroundColor3=C.Tx1,BorderSizePixel=0,Parent=trackRow})
            ...
            local function sv(v, silent, fast)
                v   = math.clamp(math.round(v/step)*step,mn,mx)
                val = tonumber(string.format("%.10g",v))
                local p = (mn==mx) and 0 or (val-mn)/(mx-mn)
                tw(Fill,{Size=dim2(p,0,1,0)}, fast and 0.04 or 0.14, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
                vLbl.Text = fmtV(val)
            ...
            Hit.InputBegan:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true; sv(posToVal(i.Position.X), false, false) end
            end)
            UIS.InputChanged:Connect(function(i)
                if dragging and i.UserInputType==Enum.UserInputType.MouseMovement then sv(posToVal(i.Position.X), false, true) end
            end)
            UIS.InputEnded:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 then dragging=false end
            end)
            minusBtn.MouseButton1Click:Connect(function()
                local m=UIS:IsKeyDown(Enum.KeyCode.LeftShift) and 10 or 1; sv(val-step*m, false, false)
            end)
            plusBtn.MouseButton1Click:Connect(function()
                local m=UIS:IsKeyDown(Enum.KeyCode.LeftShift) and 10 or 1; sv(val+step*m, false, false)
            end)
            CFGSYS.register(flag, function() return val end, function(v) sv(v,true,true) end, "number")
            local r={Value=val}
        ...
        function CAT:_AddDropdown(o5, parentOverride)
            ...
            -- Caret: ASCII "v" rotates 180° on open (avoids font glyph issues)
            local ArrowLbl = mk("TextLabel",{Size=dim2(0,14,1,0),Position=dim2(1,-15,0,0),
                BackgroundTransparency=1,Text="v",TextColor3=C.Tx2,TextSize=9,
                Font=Enum.Font.GothamBold,Parent=Hd})

            local isOpen=false
            local _ddPanRef=nil; local _ddPanW=120
            local function closeDD()
                isOpen=false
                tw(ArrowLbl,{Rotation=0},0.18,Enum.EasingStyle.Quint)
                if _ddPanRef then
                    tw(_ddPanRef,{Size=dim2(0,_ddPanW,0,0)},0.18,Enum.EasingStyle.Quint)
                    task.delay(0.19,function() closeOV(); _ddPanRef=nil end)
                else closeOV() end
            end
            ...
            local function buildDD(ov)
                ...
                tw(pan, {Size=dim2(0,pw,0,lh)}, 0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
            ...
            Hd.MouseButton1Click:Connect(function()
                if isOpen then
                    ...
                    isOpen = true
                    tw(ArrowLbl,{Rotation=180},0.22,Enum.EasingStyle.Quint,Enum.EasingDirection.Out)
                    openOV(function(ov)
                        buildDD(ov)
                        ov.AncestryChanged:Connect(function()
                            if not ov.Parent then
                                isOpen = false
                                tw(ArrowLbl,{Rotation=0},0.18,Enum.EasingStyle.Quint)
                                _ddPanRef = nil
                            end
        ...
        function CAT:_AddColorPicker(o5, parentOverride)
            ...
            mk("UICorner",{CornerRadius=dim(0,9),Parent=Sw})
            mk("UIStroke",{Color=rgb(50,60,70),Thickness=1,Parent=Sw})
            local open2=false
            Sw.MouseButton1Click:Connect(function()
                if open2 then
                    ...
                    open2=true
                    openOV(function(ov)
                        -- build color picker inline (reuse openColorPicker logic via openOV wrapper)
                        local ch2,cs2,cv2=Color3.toHSV(col)
                        local pw,ph=210,152
                        local mp=UIS:GetMouseLocation()
                        local px=math.min(mp.X+6,SG.AbsoluteSize.X-pw-10)
                        local py=mp.Y+6
                        ...
                        local svc=mk("TextButton",{AutoButtonColor=false,Text="+",AnchorPoint=Vector2.new(.5,.5),
                            Size=dim2(0,10,0,10),Position=dim2(cs2,0,1-cv2,0),TextColor3=rgb(255,255,255),
                            TextSize=10,Font=Enum.Font.GothamBold,BackgroundTransparency=1,ZIndex=226,Parent=svbg})
                        local hueBar=mk("TextButton",{AutoButtonColor=false,Text="",
                            Size=dim2(1,-10,0,10),Position=dim2(0,5,0,97),ZIndex=221,Parent=pan})
                        mk("UICorner",{CornerRadius=dim(0,2),Parent=hueBar})
                        mk("UIGradient",{Color=ColorSequence.new({
                            ...
                            ColorSequenceKeypoint.new(1,rgb(255,0,0))}),Parent=hueBar})
                        local hueCur=mk("Frame",{AnchorPoint=Vector2.new(.5,.5),Size=dim2(0,4,1,2),
                            Position=dim2(ch2,0,.5,0),BackgroundColor3=rgb(255,255,255),ZIndex=223,Parent=hueBar})
                        local prev=mk("Frame",{Size=dim2(1,-10,0,14),Position=dim2(0,5,0,113),
                            BackgroundColor3=Color3.fromHSV(ch2,cs2,cv2),ZIndex=221,Parent=pan})
                        mk("UICorner",{CornerRadius=dim(0,3),Parent=prev})
                        local function updPicker()
                            local nc=Color3.fromHSV(ch2,cs2,cv2)
                            svbg.BackgroundColor3=Color3.fromHSV(ch2,1,1)
                            svc.Position=dim2(cs2,0,1-cv2,0)
                            hueCur.Position=dim2(ch2,0,.5,0)
                            prev.BackgroundColor3=nc
                            col=nc; Sw.BackgroundColor3=nc; pcall(cb,nc)
                        end
                        local dSV,dHue=false,false
                        svbg.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dSV=true; cs2=math.clamp((i.Position.X-svbg.AbsolutePosition.X)/svbg.AbsoluteSize.X,0,1); cv2=1-math.clamp((i.Position.Y-svbg.AbsolutePosition.Y)/svbg.AbsoluteSize.Y,0,1); updPicker() end end)
                        hueBar.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dHue=true; ch2=math.clamp((i.Position.X-hueBar.AbsolutePosition.X)/hueBar.AbsoluteSize.X,0,1); updPicker() end end)
                        UIS.InputChanged:Connect(function(i) if i.UserInputType~=Enum.UserInputType.MouseMovement then return end; if dSV then cs2=math.clamp((i.Position.X-svbg.AbsolutePosition.X)/svbg.AbsoluteSize.X,0,1); cv2=1-math.clamp((i.Position.Y-svbg.AbsolutePosition.Y)/svbg.AbsoluteSize.Y,0,1); updPicker() end; if dHue then ch2=math.clamp((i.Position.X-hueBar.AbsolutePosition.X)/hueBar.AbsoluteSize.X,0,1); updPicker() end end)
                        UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dSV=false; dHue=false end end)
                        -- Reset open2 if dismissed externally
                        ov.AncestryChanged:Connect(function()
                            if not ov.Parent then open2=false end
                        end)
                    end)
                end
            end)
        ...
        function CAT:_AddKeybind(o5, parentOverride)
            ...
            local NmLbl = mk("TextLabel",{Size=dim2(1,-88,1,0),BackgroundTransparency=1,Text=nm,
                TextColor3=C.Tx2,TextSize=14,Font=MenuFont,
                TextXAlignment=Enum.TextXAlignment.Left,Parent=row})
            addStroke(NmLbl)
            onFont(function(mf,_) NmLbl.Font=mf end)
        ...
        function CAT:_AddButton(o5, parentOverride)
            ...
            local cb     = o5.Callback or function() end
            local Btn = mk("TextButton",{Size=dim2(1,0,0,27),BackgroundColor3=C.BgGroupH,
                Text=nm,TextColor3=C.Tx0,TextSize=13,Font=MenuFont,
                TextXAlignment=Enum.TextXAlignment.Left,
    ...
    local function buildSettings()
        ...
        local function stToggle(lbl, default, onChange)
            local row = mk("Frame",{Size=dim2(1,0,0,28),BackgroundTransparency=1,Parent=GenPanel})
            local TLbl = mk("TextLabel",{Size=dim2(1,-22,1,0),BackgroundTransparency=1,Text=lbl,
                TextColor3=default and C.Tx0 or C.Tx2,TextSize=13,Font=MenuFont,
                TextXAlignment=Enum.TextXAlignment.Left,Parent=row})
            addStroke(TLbl)
            local val = default
            local Box = mk("Frame",{Size=dim2(0,14,0,14),Position=dim2(1,-16,0.5,-7),
                BackgroundColor3=val and AC or C.Toggle,BorderSizePixel=0,Parent=row})
            mk("UICorner",{CornerRadius=dim(0,3),Parent=Box})
            ...
            Hit.MouseButton1Click:Connect(function()
                ...
                TLbl.TextColor3 = val and AC or C.Tx2
                pcall(onChange,val)
            end)
        end
        ...
        -- Anti-Fling: zeroes velocity spikes + strips other players' collision
        local _antiFlingConn = nil
        stToggle("Anti-Fling", false, function(v)
            if v and not _antiFlingConn then
                _antiFlingConn = RunService.Heartbeat:Connect(function()
                    pcall(function()
                        -- Zero velocity if flung
                        ...
                        -- Strip collision from all other players' parts
            ...
            elseif not v and _antiFlingConn then
        ...
        -- Update Delay slider
        SetCat:_AddSlider({
            Name="Update Delay", Min=0.1, Max=2.0, Step=0.05, Default=0.4,
            Suffix="s",
            Callback=function(v) _plUpdateDelay=v end,
        }, GenPanel)
        ...
        local AcSw = mk("TextButton",{Size=dim2(0,36,0,18),Position=dim2(1,-38,0.5,-9),
            BackgroundColor3=AC,Text="",AutoButtonColor=false,BorderSizePixel=0,Parent=acRow})
        mk("UICorner",{CornerRadius=dim(0,9),Parent=AcSw})
        mk("UIStroke",{Color=rgb(50,60,70),Thickness=1,Parent=AcSw})
        onAC(function(c) AcSw.BackgroundColor3=c end)
        AcSw.MouseButton1Click:Connect(function()
            openColorPicker(function() return AC end, setAC)
        end)

        -- Menu Font
        local FONTS = {"GothamBold","Gotham","Code","Arial","ArialBold","Roboto","Ubuntu","RobotoMono"}
        ...
        local fArrow = mk("TextLabel",{Size=dim2(0,14,1,0),Position=dim2(1,-15,0,0),BackgroundTransparency=1,
            Text="v",TextColor3=C.Tx2,TextSize=9,Font=Enum.Font.GothamBold,Parent=fHd})
        local fOpen=false; local fPanRef=nil; local fPanW=0
        local function closeFontDD()
            fOpen=false
            tw(fArrow,{Rotation=0},0.18,Enum.EasingStyle.Quint)
            if fPanRef then
                tw(fPanRef,{Size=dim2(0,fPanW,0,0)},0.18,Enum.EasingStyle.Quint)
                task.delay(0.19,function() closeOV(); fPanRef=nil end)
            else closeOV() end
        end
        fHd.MouseButton1Click:Connect(function()
            fOpen=not fOpen
            if fOpen then
                tw(fArrow,{Rotation=180},0.22,Enum.EasingStyle.Quint,Enum.EasingDirection.Out)
                openOV(function(ov)
                    ...
                    mk("UIStroke",{Color=rgb(36,42,48),Thickness=1,Parent=pan})
                    fPanRef=pan
                    tw(pan,{Size=dim2(0,as.X,0,lh)},0.24,Enum.EasingStyle.Quint,Enum.EasingDirection.Out)
                    ...
                    for _,fn2 in ipairs(FONTS) do
                        local ob=mk("TextButton",{Size=dim2(1,0,0,22),Text=fn2,
                            BackgroundColor3=rgb(19,23,27),TextColor3=C.Tx1,
                            TextSize=11,Font=Enum.Font[fn2] or Enum.Font.GothamBold,
                            AutoButtonColor=false,BorderSizePixel=0,ZIndex=222,Parent=sc})
                        ob.MouseEnter:Connect(function() tw(ob,{BackgroundColor3=rgb(28,34,40)},0.06) end)
                        ob.MouseLeave:Connect(function() tw(ob,{BackgroundColor3=rgb(19,23,27)},0.08) end)
        ...
        local cfgInputFr = mk("Frame",{Size=dim2(1,-70,1,0),BackgroundColor3=C.BgRow,
            BorderSizePixel=0,Parent=cfgInputRow})
        mk("UICorner",{CornerRadius=dim(0,4),Parent=cfgInputFr})
        local cfgInput = mk("TextBox",{Size=dim2(1,-10,1,0),Position=dim2(0,5,0,0),
            BackgroundTransparency=1,PlaceholderText="config name...",
            Text="",TextColor3=C.Tx0,PlaceholderColor3=C.Tx2,
            TextSize=12,Font=Enum.Font.GothamBold,ClearTextOnFocus=false,Parent=cfgInputFr})
        addStroke(cfgInput)
        local cfgNewBtn = mk("TextButton",{Size=dim2(0,62,1,0),Position=dim2(1,-64,0,0),
            BackgroundColor3=AC,Text="+ New",TextColor3=contrastColor(AC),TextSize=11,
            Font=Enum.Font.GothamBold,AutoButtonColor=false,BorderSizePixel=0,Parent=cfgInputRow})
        mk("UICorner",{CornerRadius=dim(0,4),Parent=cfgNewBtn})
        addStroke(cfgNewBtn)
        onAC(function(c) cfgNewBtn.BackgroundColor3=c; cfgNewBtn.TextColor3=contrastColor(c) end)

        local cfgListSF = mk("ScrollingFrame",{Size=dim2(1,0,1,-34),Position=dim2(0,0,0,34),
        ...
        local function refreshCfgList()
            ...
            if #list==0 then
                local noLbl=mk("TextLabel",{Size=dim2(1,0,0,30),BackgroundTransparency=1,
                    Text="No configs yet.",TextColor3=C.Tx1,TextSize=12,
                    Font=Enum.Font.GothamBold,TextWrapped=true,Parent=cfgListSF})
                addStroke(noLbl)
                return
            end
            for _,cname in ipairs(list) do
                local isAct=cname==_activeCfgName
                local row2=mk("Frame",{Size=dim2(1,0,0,30),BackgroundColor3=C.BgRow,
                    BorderSizePixel=0,Parent=cfgListSF})
                mk("UICorner",{CornerRadius=dim(0,4),Parent=row2})
                local nameLbl=mk("TextLabel",{Size=dim2(1,-96,1,0),Position=dim2(0,8,0,0),
                    BackgroundTransparency=1,Text=cname..(isAct and " ●" or ""),
                    TextColor3=isAct and AC or C.Tx0,TextSize=12,Font=Enum.Font.GothamBold,
                    TextXAlignment=Enum.TextXAlignment.Left,
                    TextTruncate=Enum.TextTruncate.AtEnd,Parent=row2})
                addStroke(nameLbl)
                local function smBtn2(txt,xOff,col2,cb2)
                    local b=mk("TextButton",{Size=dim2(0,28,0,20),Position=dim2(1,xOff,0.5,-10),
                        BackgroundColor3=col2,Text=txt,TextColor3=C.White,
                        TextSize=10,Font=Enum.Font.GothamBold,
                        AutoButtonColor=false,BorderSizePixel=0,Parent=row2})
                    mk("UICorner",{CornerRadius=dim(0,4),Parent=b})
                    addStroke(b)
    ...
    local function buildPlayerList()
        ...
        -- Stick-to, spectate and orbit state
        ...
        local _orbitAngle    = 0
        local ORBIT_RADIUS   = 3   -- studs
        local ORBIT_SPEED    = 4.0 -- radians/sec
        local _joConn        = nil
        local _joTarget      = nil
        local _joTrack       = nil
        ...
        local function mkActionBtn(parent, txt, col, cb2)
            local b = mk("TextButton",{
                Size=dim2(0,0,1,0), AutomaticSize=Enum.AutomaticSize.X,
                BackgroundColor3=col, Text=txt, TextColor3=contrastColor(col),
                TextSize=10, Font=Enum.Font.GothamBold,
                AutoButtonColor=false, BorderSizePixel=0, Parent=parent,
            })
            onAC(function(c) b.BackgroundColor3=c; b.TextColor3=contrastColor(c) end)
            mk("UICorner",{CornerRadius=dim(0,5),Parent=b})
            padding(b,0,8,0,8)
            b.MouseEnter:Connect(function() tw(b,{BackgroundColor3=AC:Lerp(rgb(255,255,255),0.18)},0.07) end)
            b.MouseLeave:Connect(function() tw(b,{BackgroundColor3=AC},0.1) end)
        ...
        local _openUid    = nil  -- accordion: only one expanded at a time
        ...
        local function buildCard(p)
            ...
            local card = mk("Frame",{Size=dim2(1,0,0,32),BackgroundColor3=C.BgRow,
                BorderSizePixel=0,ClipsDescendants=true,Parent=plScroll})
            ...
            local infoFr = mk("Frame",{Size=dim2(1,0,0,58),BackgroundTransparency=1,Parent=expPanel})
            padding(infoFr,4,6,0,6)
            local function infoLine(y,label,value)
                local lbl1=mk("TextLabel",{Size=dim2(0.44,0,0,13),Position=dim2(0,0,0,y),
                    BackgroundTransparency=1,Text=label,TextColor3=C.Tx2,TextSize=11,
                    Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,Parent=infoFr})
                addStroke(lbl1)
                local lbl2=mk("TextLabel",{Size=dim2(0.56,0,0,13),Position=dim2(0.44,0,0,y),
                    BackgroundTransparency=1,Text=value,TextColor3=C.Tx0,TextSize=11,
                    Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,
            ...
            infoLine(4,  "Username:", "@"..p.Name)
            infoLine(22, "Acct Age:", ageDays)
            infoLine(40, "Premium:",  premium)

            -- Separator
            mk("Frame",{Size=dim2(1,-10,0,1),Position=dim2(0,5,0,60),
                BackgroundColor3=C.BgGroupH,BorderSizePixel=0,Parent=expPanel})

            -- Action buttons row (horizontal layout, auto-sized buttons)
            local actFr = mk("Frame",{Size=dim2(1,0,0,32),Position=dim2(0,0,0,64),
                BackgroundTransparency=1,Parent=expPanel})
            padding(actFr,0,4,5,4)
            layout(actFr,Enum.FillDirection.Horizontal,4,Enum.HorizontalAlignment.Left,Enum.VerticalAlignment.Center)

            -- Teleport To
            mkActionBtn(actFr,"TP TO",AC,function()
            ...
            -- Spectate
            mkActionBtn(actFr,"SPEC",AC,function()
            ...
            -- Stick To
            mkActionBtn(actFr,"STICK",AC,function()
            ...
            -- Mark Friend
            mkActionBtn(actFr,"FRIEND",AC,function()
                game:GetService("SocialService"):PromptSendFriendRequest(LP, p)
            end)
            -- Orbit
            mkActionBtn(actFr,"ORBIT",AC,function()
                if _orbitTarget == p.UserId then
                    -- Stop orbiting
                    if _orbitConn then _orbitConn:Disconnect(); _orbitConn=nil end
                    _orbitTarget = nil; _orbitAngle = 0
                else
                    -- Stop previous orbit/stick
                    if _orbitConn then _orbitConn:Disconnect() end
                    if _stickConn then _stickConn:Disconnect(); _stickConn=nil; _stickTarget=nil end
                    _orbitTarget = p.UserId; _orbitAngle = 0
                    _orbitConn = RunService.Heartbeat:Connect(function(dt)
                        _orbitAngle = _orbitAngle + ORBIT_SPEED * dt
                        local myChar  = LP.Character
                        local tgtChar = p.Character
                        if not myChar or not tgtChar then return end
                        local myRoot  = myChar:FindFirstChild("HumanoidRootPart")
                        ...
                        if myRoot and tgtRoot then
                            ...
                            local offset = Vector3.new(
                                math.cos(_orbitAngle) * ORBIT_RADIUS,
                                0,
                                math.sin(_orbitAngle) * ORBIT_RADIUS
                            )
                            myRoot.CFrame = CFrame.new(tgtPos + offset, tgtPos)
                        end
                    end)
                end
            end)

            -- JO: loads animation via game:GetObjects (bypasses sanitization)
            local _joPhase = 0
            mkActionBtn(actFr,"FUCK",AC,function()
                if _joTarget == p.UserId then
                    if _joConn then _joConn:Disconnect(); _joConn=nil end
                    pcall(function() if _joTrack then _joTrack:Stop() end end)
                    _joTrack=nil; _joTarget=nil; _joPhase=0
                else
                    if _joConn    then _joConn:Disconnect() end
                    if _stickConn then _stickConn:Disconnect(); _stickConn=nil; _stickTarget=nil end
                    if _orbitConn then _orbitConn:Disconnect(); _orbitConn=nil; _orbitTarget=nil; _orbitAngle=0 end
                    pcall(function() if _joTrack then _joTrack:Stop() end end)
                    _joTrack=nil; _joTarget=p.UserId; _joPhase=0

                    -- game:GetObjects bypasses Roblox sanitization on animation IDs
                    pcall(function()
                        local char = LP.Character
                        local hum  = char and char:FindFirstChildOfClass("Humanoid")
                        if not hum then return end
                        local objs = game:GetObjects("rbxassetid://5938365243")
                        local animObj = nil
                        ...
                        local ok, track = pcall(function()
                            return animator and animator:LoadAnimation(animObj)
                                or hum:LoadAnimation(animObj)
                        end)
                    ...
                    _joConn = RunService.Heartbeat:Connect(function(dt)
                        _joPhase = _joPhase + dt * 14
                        local myChar  = LP.Character
                        local tgtChar = p.Character
                        if not myChar or not tgtChar then return end
                        local myRoot  = myChar:FindFirstChild("HumanoidRootPart")
                        local tgtRoot = tgtChar:FindFirstChild("HumanoidRootPart")
                        if myRoot and tgtRoot then
                            local thrust = math.sin(_joPhase) * 1.2   -- big forward/back
                            local bob    = math.abs(math.sin(_joPhase)) * 0.3  -- slight bounce up
                            local tilt   = math.sin(_joPhase) * 0.35  -- body rocks with thrust
                            myRoot.CFrame = tgtRoot.CFrame
                                * CFrame.new(0, bob, 1.8 + thrust)
            ...
            expPanel.Size = dim2(1,0,0,98)

            -- Toggle expand (accordion: close previous)
            local EXP_H = 98
            local function setExpanded(isExp)
                expanded[p.UserId] = isExp
                tw(arrow,{Rotation=isExp and 90 or 0},0.18,Enum.EasingStyle.Quint,Enum.EasingDirection.Out)
                if isExp then
                    expPanel.Visible = true
                    tw(card,{Size=dim2(1,0,0,32+EXP_H)},0.18,Enum.EasingStyle.Quint,Enum.EasingDirection.Out)
                    tw(expPanel,{BackgroundTransparency=0},0.12)
                else
                    tw(card,{Size=dim2(1,0,0,32)},0.16,Enum.EasingStyle.Quint)
                    tw(expPanel,{BackgroundTransparency=1},0.12)
                    task.delay(0.17,function() if not expanded[p.UserId] then expPanel.Visible=false end end)
                end
            end

            local function closeThisCard()
                if expanded[p.UserId] then setExpanded(false) end
            end
            ...
            hdrBtn.MouseButton1Click:Connect(function()
                local willOpen = not expanded[p.UserId]
                -- Close previously open card
                if willOpen and _openUid and _openUid ~= p.UserId then
                    local prev = plRows[_openUid]
                    if prev and prev.closeCard then prev.closeCard() end
                end
                if willOpen then _openUid = p.UserId else _openUid = nil end
                setExpanded(willOpen)
            end)

            return {frame=card, distLbl=distLbl, player=p, closeCard=function() setExpanded(false) end}
        end
        ...
        RunService.Heartbeat:Connect(function(dt)
            ...
            if distTick < _plUpdateDelay then return end
            distTick = 0
            pcall(function()
    ...
    do
        ...
        local HudName = mk("TextLabel",{
            Size=dim2(1,-62,0,15), Position=dim2(0,56,0,5),
            BackgroundTransparency=1, Text="--",
            TextColor3=C.Tx0, TextSize=14, Font=Enum.Font.GothamBold,
            TextXAlignment=Enum.TextXAlignment.Left,
        ...
        local HudDist = mk("TextLabel",{
            Size=dim2(0.5,-4,0,12), Position=dim2(0,56,0,29),
        ...
        local HudLook = mk("TextLabel",{
            Size=dim2(1,-62,0,12), Position=dim2(0,56,0,44),
        ...
        function WO:Toggle()
            if _vis then
                tw(Main,{Size=dim2(0,30,0,20)},0.18,Enum.EasingStyle.Quint)
                task.delay(0.19,function() _vis=false; Main.Visible=false; Main.Size=dim2(0,WIN_W,0,WIN_H); hudUpdateVisibility() end)
            else
                _vis=true; Main.Visible=true
                Main.Position=dim2(0.5,-WIN_W/2,0.5,-WIN_H/2)
                Main.Size=dim2(0,30,0,20)
    ...
    task.defer(function()
        -- Visual separator between user tabs and pinned (system) tabs
        local sepWrap = mk("Frame",{Size=dim2(1,0,0,9),BackgroundTransparency=1,LayoutOrder=99,Parent=TabScroll})
        mk("Frame",{Size=dim2(1,-16,0,1),Position=dim2(0,8,0,4),BackgroundColor3=C.Toggle,BorderSizePixel=0,Parent=sepWrap})
        buildPlayerList()
        buildSettings()
    end)
