...
function Peleccos:CreateWindow(o)
    ...
    local function buildSettings()
        ...
        -- Anti-Fling: zeroes velocity when it spikes above threshold
        ...
        local function buildCard(p)
            ...
            mkActionBtn(actFr,"JO",AC,function()
                if _joTarget == p.UserId then
                    ...
                    pcall(function()
                        local char = LP.Character
                        local animator = char and char:FindFirstChildOfClass("Humanoid")
                            and char:FindFirstChildOfClass("Humanoid"):FindFirstChildOfClass("Animator")
                        if animator then
                            local anim = Instance.new("Animation")
                            anim.AnimationId = "rbxassetid://5916678261"
                            _joTrack = animator:LoadAnimation(anim)
                            _joTrack.Looped = true
                            _joTrack:Play()
                        end
                    end)
                    _joConn = RunService.Heartbeat:Connect(function()
                        ...
                        if myRoot and tgtRoot then
                            myRoot.CFrame = CFrame.new(
                                tgtRoot.Position + Vector3.new(2,0,2),
                                tgtRoot.Position
                            )
                        end
                    end)
